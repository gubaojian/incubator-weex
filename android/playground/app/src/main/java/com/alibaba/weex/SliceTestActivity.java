...
public class SliceTestActivity extends AppCompatActivity {
  ...
  private WXViewHolder mFirst = null;
  ...
  public void addCellClick(View view) {
    ...
    if(obj.getJSONObject("status").getIntValue("layoutStyle") == 1){
  ...
  public void refreshCellClick(View view) {

    if (mFirst==null){
      Toast.makeText(this, "First holder not exist", Toast.LENGTH_SHORT).show();
    } else {

      JSONObject obj = JSON.parseObject(mData.get(0));
      if(obj.getJSONObject("status").getIntValue("layoutStyle") == 1){
        obj.getJSONObject("status").put("layoutStyle",0);
      } else {
        obj.getJSONObject("status").put("layoutStyle",1);
      }
      mData.set(0,obj.toJSONString());
      mFirst.refresh(obj.toJSONString(), 0);
    }
  }
  ...
  private class WXViewHolder extends RecyclerView.ViewHolder implements IWXRenderListener {
    ...
    public void onRenderSuccess(WXSDKInstance instance, int width, int height) {
      Log.e("SliceTestActivity","render succ " );
    }

    @Override
    public void onRefreshSuccess(WXSDKInstance instance, int width, int height) {
      Log.e("SliceTestActivity","refresh succ " );
    }
