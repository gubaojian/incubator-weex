...
public class FontWatchTask implements  Runnable{
    ...
    private List<RenderFrame> renderFrames;
    ...
    public FontWatchTask(String fontFaimly, String fontUrl) {
        ...
        this.renderFrames = new ArrayList<>();
        this.isLoaded = false;
    }
    ...
    public void run() {
        ...
        if(new File(fontPath).exists()){
            if(!RenderBridge.getInstance().hasFont(fontFaimly, fontPath)){
                ...
                synchronized (renderFrames){
                    for(RenderFrame renderFrame : renderFrames){
                        renderFrame.actionRefreshFont(fontFaimly);
                    }
                }
            }
        }
    ...
    public void addWatchDocument(RenderFrame documentView){
        if(isLoaded){
            return;
        }
        synchronized (renderFrames){
            renderFrames.add(documentView);
        }
    }
