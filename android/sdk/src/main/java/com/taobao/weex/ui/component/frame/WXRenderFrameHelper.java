 ...
 */
public class WXRenderFrameHelper {

    private WXRenderFrameComponent documentComponent;
    private boolean isAppear;
    private Handler mainHandler;
    private Runnable updateWatchEventsRunnable;


    public WXRenderFrameHelper(WXRenderFrameComponent documentComponent) {
    ...
    private void notifyDocumentChildAppearEvent(WXComponent component, String direction){
        if(isAppear){
            component.frameNodeAppearChange(Constants.Event.APPEAR, direction);
        }else{
            component.frameNodeAppearChange(Constants.Event.DISAPPEAR, direction);
        }
        if(component instanceof WXVContainer){
            WXVContainer container = (WXVContainer) component;
            for(int i=0; i<container.getChildCount(); i++){
