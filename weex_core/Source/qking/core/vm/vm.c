...
static ecma_value_t vm_op_for_in(ecma_value_t *lhs, ecma_value_t rhs) {
  ...
  do {
    ...
    ecma_collection_chunk_t *prop_names_p = ECMA_GET_POINTER(
        ecma_collection_chunk_t, prop_names_coll_p->first_chunk_cp);
    if (ecma_is_value_undefined(*lhs) || ecma_is_value_null(*lhs)) {
      QKING_ASSERT(!ecma_is_value_pointer(prop_names_p->items[0]));
      *lhs = prop_names_p->items[0];
      condition = true;
    } else {
      ecma_collection_chunk_t *chunk_p = prop_names_p;
      int index = 0;
      bool finder = false;
      while (chunk_p) {
        ecma_value_t current = chunk_p->items[index];
        if (ecma_is_value_pointer(current)) {
          ecma_collection_chunk_t *next_chunk_p;
          next_chunk_p =
              (ecma_collection_chunk_t *)ecma_get_pointer_from_value(current);
        ...
        if (finder) {
          ...
          *lhs = ecma_fast_copy_value(current);
          condition = true;
          break;
        }
        if (current == *lhs) {
      ...
      if (!finder) {
        ecma_fast_free_value(*lhs);
        *lhs = ecma_fast_copy_value(prop_names_p->items[0]);
        condition = true;
      }
    }
    ecma_free_value(obj_expr_value);
                                    ...
                                    ecma_value_t property) /**< property name */
{
  if (ecma_is_value_object(object)) {
    ...
    LOGD("-- Get property: %s",debug_print_str);
                                    ...
                                    bool is_strict)        /**< strict mode */
{
  ...
  LOGD("-- Set property: %s",debug_print_str);
...
static void vm_register_loop_prev(ecma_register_t *o) {
  ecma_register_t *cur = o;
  while (cur->prev_p) {
    ecma_fast_free_value(cur->prev_p->var);
    cur->prev_p->var = ecma_fast_copy_value(cur->var);
    cur = cur->prev_p;
  }
}

static void vm_register_loop_next(ecma_register_t *o) {
  ecma_register_t *cur = o;
  while (cur->next_p) {
    ecma_fast_free_value(cur->next_p->var);
    cur->next_p->var = ecma_fast_copy_value(cur->var);
    cur = cur->next_p;
  }
}

static void vm_loop_register(ecma_register_t *a) {
  if (a->prev_p) {
    vm_register_loop_prev(a);
  }
  if (a->next_p) {
    vm_register_loop_next(a);
  }
}
#define CHECK_RET_ERR(test_value)          \
...
//          }
        ...
        case OP_CONSTRUCTOR: {
          ...
          if (!ecma_op_is_callable(c->var)) {
            a->var =
                ecma_raise_type_error(ECMA_ERR_MSG("Expected a function."));
          } else {
            switch (op) {
              case OP_CALL: {
...
//              bytecode_p->status_flags |= ECMA_CODE_FLAGS_LEXICAL_ENV_NOT_NEEDED;
...
//          LOGD("rhs:%s\n", convert_to_log_string_from_value(rhs).c_str());
          ...
          completion_value = vm_op_delete_prop(lhs, rhs, is_strict);
          if (ECMA_IS_VALUE_ERROR(completion_value)) {
            goto ret_value_handle_label;
          }
          QKING_ASSERT(ecma_is_value_boolean(completion_value));
          ecma_fast_free_value(a->var);
          a->var = completion_value;
          break;
        }
        case OP_SET_GETTER:
        case OP_SET_SETTER: {
    ...
    if (!ECMA_IS_VALUE_ERROR(completion_value)) {
      ...
      if (vm_stack_find_finally(frame_ctx_p, &context_top_p,
        ...
        if (VM_GET_CONTEXT_TYPE(context_top_p[-1]) == VM_CONTEXT_CATCH) {
          ...
          uint32_t error_reg = (uint32_t)(GET_ARG_A(pc[-1]));
          (registers_p + error_reg)->var =
              QKING_CONTEXT(error_value);
          ecma_object_t *catch_env_p =
              ecma_create_decl_lex_env(frame_ctx_p->lex_env_p);
...
static bool vm_is_out_closure_register(const ecma_compiled_function_state_t *func_state_p, int index)
{
    bool is_out_closure_register = false;
    for (int i = 0; i < func_state_p->out_closure; i++) {
        if (index == func_state_p->pp_out_closure[i]->idx) {
            is_out_closure_register = true;
            break;
        }
    }
    return is_out_closure_register;
}
...
vm_execute(vm_frame_ctx_t *frame_ctx_p, const ecma_value_t this_binding_value,
  ...
  for (uint32_t i = 0; i < register_end; i++) {
      if (register_p[i].next_p || register_p[i].prev_p) {
#ifdef QKING_ENABLE_GC_DEBUG
          if (vm_is_out_closure_register(func_state_p, i)) {
              printf("[gc][%d]=>[remove][out_closure][register][%d]\n", register_p[i].var, i);
          }
          else {
              printf("[gc][%d]=>[remove][in_closure][register][%d]\n", register_p[i].var, i);
          }
#endif
          if (vm_is_out_closure_register(func_state_p, i)) {
              if (register_p[i].next_p) {
                  ecma_op_function_remove_register_out_closure(&register_p[i]);
              }
          }
          else {
              ecma_op_function_remove_register_in_closure(&register_p[i]);
          }
      }
