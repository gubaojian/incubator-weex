...
static jmem_cpointer_t gs_cpointer_allocated[102400] = { 0 };

static uint32_t gs_cpointer_allocated_size[102400] = { 0 };

static uint32_t gs_cpointer_allocated_count = 0;

static void jmem_heap_alloc_record(void *ptr, uint32_t size) {
    jmem_cpointer_t cp_pointer = ECMA_NULL_POINTER;
    JMEM_CP_SET_NON_NULL_POINTER (cp_pointer, ptr);
    gs_cpointer_allocated[gs_cpointer_allocated_count] = cp_pointer;
    gs_cpointer_allocated_size[gs_cpointer_allocated_count] = size;
    gs_cpointer_allocated_count++;
}

static void jmem_heap_free_record(void *ptr, size_t size) {
    jmem_cpointer_t cp_pointer = ECMA_NULL_POINTER;
    JMEM_CP_SET_NON_NULL_POINTER (cp_pointer, ptr);
    for (int i = 0; i < gs_cpointer_allocated_count; i++) {
        if (gs_cpointer_allocated[i] == cp_pointer) {
            if (gs_cpointer_allocated_count - i - 1 > 0) {
                memcpy(gs_cpointer_allocated + i, gs_cpointer_allocated + i + 1, sizeof(jmem_cpointer_t) * (gs_cpointer_allocated_count - i - 1));
                memcpy(gs_cpointer_allocated_size + i, gs_cpointer_allocated_size + i + 1, sizeof(uint32_t) * (gs_cpointer_allocated_count - i - 1));
            }
            gs_cpointer_allocated_count--;
            break;
        }
...
#ifdef QKING_ENABLE_GC_DEBUG
  for (int i = 0; i < gs_cpointer_allocated_count; i++) {
    printf("[gc][%i]=>[leak][%d]\n", gs_cpointer_allocated[i], gs_cpointer_allocated_size[i]);
  }
#endif
...
#ifdef QKING_ENABLE_GC_DEBUG
  jmem_heap_alloc_record(data_space_p, (uint32_t)size);
#endif
...
#ifdef QKING_ENABLE_GC_DEBUG
  jmem_heap_free_record(ptr, (uint32_t)size);
#endif
