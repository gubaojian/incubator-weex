 ...
 * to support more than 4G address space for Qking.
 */
...
void jmem_heap_check(void);
#endif

void *jmem_system_malloc(size_t size);
void jmem_system_free(void *ptr);
...
typedef struct {
  ...
  size_t
      skip_count; /**< Number of skip-aheads during insertion of free block */
  ...
  size_t free_iter_count;  /**< Number of iterations required for inserting free
                              blocks */

  size_t c_alloc_free_remain; /**< currently allocated bytes */
} jmem_heap_stats_t;
...
jmem_cpointer_t QKING_ATTR_PURE jmem_compress_pointer(const void *pointer_p);
void *QKING_ATTR_PURE jmem_decompress_pointer(uintptr_t compressed_pointer);

/**
 * A free memory callback routine type.
 */
typedef void (*jmem_free_unused_memory_callback_t)(
    jmem_free_unused_memory_severity_t);

void jmem_register_free_unused_memory_callback(
    jmem_free_unused_memory_callback_t callback);
void jmem_unregister_free_unused_memory_callback(
    jmem_free_unused_memory_callback_t callback);

void jmem_run_free_unused_memory_callbacks(
    jmem_free_unused_memory_severity_t severity);
 ...
 *
 * Warning:
 *         if there is not enough memory on the heap, shutdown engine with
 * ERR_OUT_OF_MEMORY.
 */
...
/**
 * Free the previously defined local array variable, freeing corresponding block
 * on the heap, if it was allocated (i.e. if the array's size was non-zero).
 */
#define JMEM_FINALIZE_LOCAL_ARRAY(var_name)            \
  if (var_name != NULL) {                              \
    QKING_ASSERT(var_name##___size != 0);              \
                                                       \
...
#define JMEM_CP_GET_POINTER(type, cp_value)      \
  (((QKING_UNLIKELY((cp_value) == JMEM_CP_NULL)) \
        ? NULL                                   \
        : JMEM_CP_GET_NON_NULL_POINTER(type, cp_value)))
...
#define JMEM_CP_SET_POINTER(cp_value, non_compressed_pointer) \
  do {                                                        \
    void *ptr_value = (void *)non_compressed_pointer;         \
                                                              \
    if (QKING_UNLIKELY((ptr_value) == NULL)) {                \
