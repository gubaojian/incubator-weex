...
#define CONFIG_MEM_HEAP_DESIRED_LIMIT \
  (QKING_MIN(CONFIG_MEM_HEAP_AREA_SIZE / 32, CONFIG_MEM_HEAP_MAX_LIMIT))
...
/**
 * Share of newly allocated since last GC objects among all currently allocated
 * objects, after achieving which, GC is started upon low severity
 * try-give-memory-back requests.
 *
