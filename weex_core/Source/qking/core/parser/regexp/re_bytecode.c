...
{
  QKING_ASSERT (bc_ctx_p->block_end_p >= bc_ctx_p->block_start_p);
  ...
  QKING_ASSERT ((!bc_ctx_p->current_p && !bc_ctx_p->block_end_p && !bc_ctx_p->block_start_p)
                || (bc_ctx_p->current_p && bc_ctx_p->block_end_p && bc_ctx_p->block_start_p));

  size_t new_block_size = old_size + REGEXP_BYTECODE_BLOCK_SIZE;
  QKING_ASSERT (bc_ctx_p->current_p >= bc_ctx_p->block_start_p);
...
{
  QKING_ASSERT (length <= REGEXP_BYTECODE_BLOCK_SIZE);
 ...
 */
inline re_opcode_t QKING_ATTR_ALWAYS_INLINE
 ...
 */
inline uint32_t QKING_ATTR_PURE QKING_ATTR_ALWAYS_INLINE
...
re_dump_bytecode (re_bytecode_ctx_t *bc_ctx_p) /**< RegExp bytecode context */
{
  ...
  QKING_DEBUG_MSG ("%d ", compiled_code_p->header.status_flags);
  QKING_DEBUG_MSG ("%d ", compiled_code_p->num_of_captures);
  QKING_DEBUG_MSG ("%d | ", compiled_code_p->num_of_non_captures);
  ...
  while ((op = re_get_opcode (&bytecode_p)))
  {
    switch (op)
    {
      case RE_OP_MATCH:
      {
        QKING_DEBUG_MSG ("MATCH, ");
        break;
      }
      case RE_OP_CHAR:
      {
        QKING_DEBUG_MSG ("CHAR ");
        QKING_DEBUG_MSG ("%c, ", (char) re_get_char (&bytecode_p));
        break;
      }
      ...
      {
        QKING_DEBUG_MSG ("GZ_START ");
        QKING_DEBUG_MSG ("%d ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      ...
      {
        QKING_DEBUG_MSG ("RE_START ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      case RE_OP_SAVE_AND_MATCH:
      {
        QKING_DEBUG_MSG ("RE_END, ");
        break;
      }
      case RE_OP_GREEDY_ITERATOR:
      {
        QKING_DEBUG_MSG ("GREEDY_ITERATOR ");
        QKING_DEBUG_MSG ("%d ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      ...
      {
        QKING_DEBUG_MSG ("NON_GREEDY_ITERATOR ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      ...
      case RE_OP_PERIOD:
      {
        QKING_DEBUG_MSG ("PERIOD ");
        break;
      }
      case RE_OP_ALTERNATIVE:
      {
        QKING_DEBUG_MSG ("ALTERNATIVE ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      case RE_OP_ASSERT_START:
      {
        QKING_DEBUG_MSG ("ASSERT_START ");
        break;
      }
      case RE_OP_ASSERT_END:
      {
        QKING_DEBUG_MSG ("ASSERT_END ");
        break;
      }
      case RE_OP_ASSERT_WORD_BOUNDARY:
      {
        QKING_DEBUG_MSG ("ASSERT_WORD_BOUNDARY ");
        break;
      }
      case RE_OP_ASSERT_NOT_WORD_BOUNDARY:
      {
        QKING_DEBUG_MSG ("ASSERT_NOT_WORD_BOUNDARY ");
        break;
      }
      case RE_OP_LOOKAHEAD_POS:
      {
        QKING_DEBUG_MSG ("LOOKAHEAD_POS ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      case RE_OP_LOOKAHEAD_NEG:
      {
        QKING_DEBUG_MSG ("LOOKAHEAD_NEG ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      case RE_OP_BACKREFERENCE:
      {
        QKING_DEBUG_MSG ("BACKREFERENCE ");
        QKING_DEBUG_MSG ("%d, ", re_get_value (&bytecode_p));
        break;
      }
      case RE_OP_INV_CHAR_CLASS:
      {
        QKING_DEBUG_MSG ("INV_");
        /* FALLTHRU */
      }
      case RE_OP_CHAR_CLASS:
      {
        QKING_DEBUG_MSG ("CHAR_CLASS ");
        uint32_t num_of_class = re_get_value (&bytecode_p);
        QKING_DEBUG_MSG ("%d", num_of_class);
        while (num_of_class)
        {
          QKING_DEBUG_MSG (" %d", re_get_char (&bytecode_p));
          QKING_DEBUG_MSG ("-%d", re_get_char (&bytecode_p));
          num_of_class--;
        }
        QKING_DEBUG_MSG (", ");
      ...
      default:
      {
        QKING_DEBUG_MSG ("UNKNOWN(%d), ", (uint32_t) op);
        break;
      }
    }
  }
