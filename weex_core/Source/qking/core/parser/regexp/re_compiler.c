                      ...
                      bool expect_eof) /**< expect end of file */
{
  ...
  while (ecma_is_value_empty (ret_value) && should_loop)
  {
    ...
    QKING_ASSERT (next_token_result == ECMA_VALUE_EMPTY);
    ...
    switch (re_ctx_p->current_token.type)
    {
      case RE_TOK_START_CAPTURE_GROUP:
      {
        ...
        QKING_TRACE_MSG ("Compile a capture group start (idx: %u)\n", (unsigned int) idx);
      ...
      case RE_TOK_START_NON_CAPTURE_GROUP:
      {
        ...
        QKING_TRACE_MSG ("Compile a non-capture group start (idx: %u)\n", (unsigned int) idx);
      ...
      case RE_TOK_CHAR:
      {
        QKING_TRACE_MSG ("Compile character token: %c, qmin: %u, qmax: %u\n",
      ...
      case RE_TOK_PERIOD:
      {
        QKING_TRACE_MSG ("Compile a period\n");
      ...
      {
        QKING_TRACE_MSG ("Compile an alternative\n");
      ...
      {
        QKING_TRACE_MSG ("Compile a negative lookahead assertion\n");
        idx = re_ctx_p->num_of_non_captures++;
      ...
      case RE_TOK_BACKREFERENCE:
      {
        ...
        QKING_TRACE_MSG ("Compile a backreference: %u\n", (unsigned int) backref);
      ...
      {
        QKING_TRACE_MSG ("Compile a character class\n");
      ...
      {
        QKING_TRACE_MSG ("Compile a group end\n");
                           ...
                           uint16_t flags) /**< flags */
{
  ...
  {
    const re_compiled_code_t *cached_bytecode_p = QKING_CONTEXT (re_cache)[idx];
    ...
    if (cached_bytecode_p != NULL)
    {
      ...
      {
        QKING_TRACE_MSG ("RegExp is found in cache\n");
        return idx;
      }
    }
    else
  ...
  QKING_TRACE_MSG ("RegExp is NOT found in cache\n");
  return free_idx;
...
re_cache_gc_run (void)
{
  ...
  {
    const re_compiled_code_t *cached_bytecode_p = QKING_CONTEXT (re_cache)[i];
                     ...
                     uint16_t flags) /**< flags */
{
  ...
  {
    *out_bytecode_p = QKING_CONTEXT (re_cache)[cache_idx];
  ...
  if (!ecma_is_value_empty (ret_value))
  {
    ...
    QKING_TRACE_MSG ("RegExp compilation failed!\n");
  ...
  else
  {
    ...
    if (QKING_CONTEXT (qking_init_flags) & ECMA_INIT_SHOW_REGEXP_OPCODES)
    {
    ...
    QKING_ASSERT (bc_ctx.block_start_p != NULL);
    *out_bytecode_p = (re_compiled_code_t *) bc_ctx.block_start_p;
    ...
    if (cache_idx == RE_CACHE_SIZE)
    {
      if (QKING_CONTEXT (re_cache_idx) == RE_CACHE_SIZE)
      {
        QKING_CONTEXT (re_cache_idx) = 0;
      }

      QKING_TRACE_MSG ("RegExp cache is full! Remove the element on idx: %d\n", QKING_CONTEXT (re_cache_idx));

      cache_idx = QKING_CONTEXT (re_cache_idx)++;

      /* The garbage collector might run during the byte code
       * allocations above and it may free this entry. */
      if (QKING_CONTEXT (re_cache)[cache_idx] != NULL)
      {
        ecma_bytecode_deref ((ecma_compiled_code_t *) QKING_CONTEXT (re_cache)[cache_idx]);
      }
    }

    QKING_TRACE_MSG ("Insert bytecode into RegExp cache (idx: %d).\n", cache_idx);
    ecma_bytecode_ref ((ecma_compiled_code_t *) *out_bytecode_p);
    QKING_CONTEXT (re_cache)[cache_idx] = *out_bytecode_p;
  }
