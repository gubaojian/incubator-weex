...
/**
 * RegExp flags mask (first 10 bits are for reference count and the rest for the
 * actual RegExp flags)
 */
...
typedef enum {
  RE_OP_EOF,
  /* Group opcode order is important, because RE_IS_CAPTURE_GROUP is based on
   * it. Change it carefully. Capture opcodes should be at first.
   */
  ...
  RE_OP_NON_CAPTURE_GREEDY_ZERO_GROUP_START, /**< non-capture greedy zero group
                                                start */
  RE_OP_NON_CAPTURE_NON_GREEDY_ZERO_GROUP_START, /**< non-capture non-greedy
                                                    zero group start */
...
uint32_t QKING_ATTR_PURE re_get_bytecode_length(re_bytecode_ctx_t *bc_ctx_p);
...
void re_insert_opcode(re_bytecode_ctx_t *bc_ctx_p, uint32_t offset,
                      re_opcode_t opcode);
void re_insert_u32(re_bytecode_ctx_t *bc_ctx_p, uint32_t offset,
                   uint32_t value);
void re_bytecode_list_insert(re_bytecode_ctx_t *bc_ctx_p, size_t offset,
                             uint8_t *bytecode_p, size_t length);
