...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_number_prototype.inc.h"
                                                ...
                                                lit_utf8_size_t to_num_digits) /**< requested number of digits */
{
  ...
  else
  {
    ...
    lit_utf8_size_t to_copy = QKING_MIN (num_digits, to_num_digits);
    to_copy = QKING_MIN (to_copy, (lit_utf8_size_t) exponent);
                                                ...
                                                ecma_length_t arguments_list_len) /**< number of arguments */
{
  ...
  else
  {
    ...
    else
    {
      ...
      while (whole > 0)
      {
        QKING_ASSERT (buff_index < buff_size && buff_index < required_digits);
      ...
      if (point < 0)
      {
         ...
         */
        QKING_ASSERT (exponent < 0);
        required_digits += point;
      }

      QKING_ASSERT (required_digits <= buff_size);
      ...
      QKING_ASSERT (buff_index <= buff_size);
...
ecma_builtin_number_prototype_object_value_of (ecma_value_t this_arg) /**< this argument */
{
  ...
  else if (ecma_is_value_object (this_arg))
  {
    ...
    if (ecma_object_class_is (object_p, LIT_MAGIC_STRING_NUMBER_UL))
    {
      ...
      QKING_ASSERT (ecma_is_value_number (ext_object_p->u.class_prop.u.value));
    ...
    else
    {
      ...
      else
      {
        ...
        else
        {
          ...
          QKING_ASSERT (p - buff < buffer_size);
          /* String terminator. */
          *p = 0;
    ...
    else
    {
      ...
      else
      {
        ...
        *actual_char_p++ = 'e';
        ...
        QKING_ASSERT (actual_char_p - buff < buffer_size);
        *actual_char_p = '\0';
