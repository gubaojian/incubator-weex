...
{
  QKING_ASSERT (obj_p != NULL && !ecma_is_lexical_environment (obj_p));
  QKING_ASSERT (builtin_id < ECMA_BUILTIN_ID__COUNT);

  if (QKING_CONTEXT (ecma_builtin_objects)[builtin_id] == NULL)
  {
  ...
  {
    return (obj_p == QKING_CONTEXT (ecma_builtin_objects)[builtin_id]);
  }
} /* ecma_builtin_is */
...
ecma_builtin_get (ecma_builtin_id_t builtin_id) /**< id of built-in to check on */
{
  ...
  if (QKING_UNLIKELY (QKING_CONTEXT (ecma_builtin_objects)[builtin_id] == NULL))
  {
    ecma_instantiate_builtin (builtin_id);
  }

  ecma_ref_object (QKING_CONTEXT (ecma_builtin_objects)[builtin_id]);

  return QKING_CONTEXT (ecma_builtin_objects)[builtin_id];
} /* ecma_builtin_get */
 ...
 */
inline ecma_object_t * QKING_ATTR_ALWAYS_INLINE
ecma_builtin_get_global (void)
{
  QKING_ASSERT (QKING_CONTEXT (ecma_builtin_objects)[ECMA_BUILTIN_ID_GLOBAL] != NULL);

  return QKING_CONTEXT (ecma_builtin_objects)[ECMA_BUILTIN_ID_GLOBAL];
...
{
  QKING_ASSERT (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_FUNCTION);
  QKING_ASSERT (ecma_get_object_is_builtin (func_obj_p));
                          ...
                          bool is_extensible) /**< value of object's [[Extensible]] property */
{
  ...
  if (property_count > 32)
  {
    ...
    QKING_ASSERT (property_count <= (32 + 64));
  ...
  if (QKING_UNLIKELY (!is_extensible))
  {
  ...
  switch (obj_builtin_id)
  {
    ...
    {
      QKING_ASSERT (obj_type == ECMA_OBJECT_TYPE_ARRAY);
    ...
    {
      QKING_ASSERT (obj_type == ECMA_OBJECT_TYPE_CLASS);
      ecma_extended_object_t *ext_object_p = (ecma_extended_object_t *) obj_p;
...
ecma_instantiate_builtin_helper (ecma_builtin_id_t builtin_id, /**< built-in id */
                                 ...
                                 bool is_extensible) /**< value of object's [[Extensible]] property */
{
  QKING_ASSERT (QKING_CONTEXT (ecma_builtin_objects)[builtin_id] == NULL);
  ...
  {
    if (QKING_CONTEXT (ecma_builtin_objects)[object_prototype_builtin_id] == NULL)
    {
      ecma_instantiate_builtin (object_prototype_builtin_id);
    }
    prototype_obj_p = QKING_CONTEXT (ecma_builtin_objects)[object_prototype_builtin_id];
  ...
  ecma_object_t *builtin_obj_p = ecma_builtin_init_object (builtin_id,
                                                           ...
                                                           object_type,
                                                           is_extensible);
  QKING_CONTEXT (ecma_builtin_objects)[builtin_id] = builtin_obj_p;
...
{
  QKING_ASSERT (id < ECMA_BUILTIN_ID__COUNT);
  switch (id)
  {
/** @cond doxygen_suppress */
#define BUILTIN(builtin_id, \
...
/** @endcond */
    default:
    {
      QKING_UNREACHABLE (); /* The built-in is not implemented. */
    }
  }
} /* ecma_instantiate_builtin */

/**
 * Finalize ECMA closure objects
 */
void ecma_finalize_closure(void) {
    uint32_t closure = QKING_CONTEXT (vm_func_out_closure);
    ecma_func_out_closure_t **closure_pp = QKING_CONTEXT (vm_func_out_closure_pp);
    for (int i = 0; i < closure; i++) {
        jmem_heap_free_block(closure_pp[i], sizeof(ecma_func_out_closure_t));
    }
    if (closure_pp) {
        jmem_heap_free_block(closure_pp, sizeof(ecma_func_out_closure_t *) *closure);
        QKING_CONTEXT (vm_func_out_closure_pp) = NULL;
    }
}
...
ecma_finalize_builtins (void)
{
  ...
  {
    if (QKING_CONTEXT (ecma_builtin_objects)[id] != NULL)
    {
      ecma_deref_object (QKING_CONTEXT (ecma_builtin_objects)[id]);
      QKING_CONTEXT (ecma_builtin_objects)[id] = NULL;
    }
  }
...
{
  QKING_ASSERT (length_prop_value < (1 << ECMA_BUILT_IN_BITSET_SHIFT));
  ...
  QKING_ASSERT (routine_id >= ECMA_BUILTIN_ID__COUNT);
                                          ...
                                          ecma_string_t *string_p) /**< property's name */
{
  ...
  QKING_ASSERT (builtin_id < ECMA_BUILTIN_ID__COUNT);
  QKING_ASSERT (ecma_builtin_is (object_p, builtin_id));
  ...
  *bitset_p |= bit_for_index;
  ...
  switch (curr_property_p->type)
  {
    ...
    case ECMA_BUILTIN_PROPERTY_NUMBER:
    {
      ...
      else
      {
        switch (curr_property_p->value)
        {
          ...
          default:
          {
            QKING_ASSERT (curr_property_p->value == ECMA_BUILTIN_NUMBER_NAN);
    ...
    default:
    {
      QKING_ASSERT (curr_property_p->type == ECMA_BUILTIN_PROPERTY_ACCESSOR_READ_ONLY);
...
{
  QKING_ASSERT (builtin_object_id < ECMA_BUILTIN_ID__COUNT);
...
{
  QKING_ASSERT (ecma_get_object_type (obj_p) == ECMA_OBJECT_TYPE_FUNCTION);
  QKING_ASSERT (ecma_get_object_is_builtin (obj_p));
  ...
  else
  {
    ...
    QKING_ASSERT (builtin_object_id < sizeof (ecma_builtin_call_functions) / sizeof (ecma_builtin_dispatch_call_t));
    return ecma_builtin_call_functions[builtin_object_id] (arguments_list_p, arguments_list_len);
  }

  QKING_ASSERT (!ecma_is_value_empty (ret_value));
                                 ...
                                 ecma_length_t arguments_list_len) /**< arguments list length */
{
  ...
  ecma_extended_object_t *ext_obj_p = (ecma_extended_object_t *) obj_p;
  ecma_builtin_id_t builtin_object_id = ext_obj_p->u.built_in.id;
  QKING_ASSERT (builtin_object_id < sizeof (ecma_builtin_construct_functions) / sizeof (ecma_builtin_dispatch_call_t));
