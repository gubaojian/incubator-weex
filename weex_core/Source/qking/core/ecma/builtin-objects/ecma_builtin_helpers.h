...
ecma_value_t ecma_builtin_helper_get_to_locale_string_at_index(
    ecma_object_t *obj_p, uint32_t index);
ecma_value_t ecma_builtin_helper_object_get_properties(
    ecma_object_t *obj_p, bool only_enumerable_properties);
ecma_value_t ecma_builtin_helper_array_concat_value(ecma_object_t *obj_p,
                                                    uint32_t *length_p,
                                                    ecma_value_t value);
uint32_t ecma_builtin_helper_array_index_normalize(ecma_number_t index,
                                                   uint32_t length);
uint32_t ecma_builtin_helper_string_index_normalize(ecma_number_t index,
                                                    uint32_t length,
                                                    bool nan_to_zero);
ecma_value_t ecma_builtin_helper_string_prototype_object_index_of(
    ecma_value_t this_arg, ecma_value_t arg1, ecma_value_t arg2,
    bool first_index);
bool ecma_builtin_helper_string_find_index(ecma_string_t *original_str_p,
                                           ecma_string_t *search_str_p,
                                           bool first_index,
                                           ecma_length_t start_pos,
                                           ecma_length_t *ret_index_p);
ecma_value_t ecma_builtin_helper_def_prop(ecma_object_t *obj_p,
                                          ecma_string_t *index_p,
                                          ecma_value_t value, uint32_t opts,
                                          bool is_throw);
...
#define ECMA_DATE_MS_PER_MINUTE \
  (ECMA_DATE_MS_PER_SECOND * ECMA_DATE_SECONDS_PER_MINUTE)

/** ECMA_DATE_MS_PER_HOUR == 3600000 */
#define ECMA_DATE_MS_PER_HOUR \
  (ECMA_DATE_MS_PER_MINUTE * ECMA_DATE_MINUTES_PER_HOUR)
...
ecma_number_t ecma_date_make_time(ecma_number_t hour, ecma_number_t min,
                                  ecma_number_t sec, ecma_number_t ms);
ecma_number_t ecma_date_make_day(ecma_number_t year, ecma_number_t month,
                                 ecma_number_t date);
...
typedef struct struct_ecma_json_occurence_stack_item_t {
  struct struct_ecma_json_occurence_stack_item_t
      *next_p;             /**< next stack item */
...
ecma_value_t ecma_builtin_helper_json_create_formatted_json(
...
ecma_value_t ecma_builtin_helper_error_dispatch_call(
    ecma_standard_error_t error_type, const ecma_value_t *arguments_list_p,
...
ecma_value_t ecma_builtin_helper_array_heap_sort_helper(
    ecma_value_t *array_p, uint32_t right, ecma_value_t compare_func,
