...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_string_prototype.inc.h"
...
ecma_builtin_string_prototype_object_to_string (ecma_value_t this_arg) /**< this argument */
{
  ...
  else if (ecma_is_value_object (this_arg))
  {
    ...
    if (ecma_object_class_is (object_p, LIT_MAGIC_STRING_STRING_UL))
    {
      ...
      QKING_ASSERT (ecma_is_value_string (ext_object_p->u.class_prop.u.value));
                                              ...
                                              bool charcode_mode) /**< routine mode */
{
  ...
  if (QKING_UNLIKELY (!ecma_is_value_empty (to_num_result)))
  {
   ...
   */
  QKING_ASSERT (ecma_number_is_nan (index_num) || ecma_number_to_uint32 (index_num) == ecma_number_trunc (index_num));
                                             ...
                                             ecma_length_t arguments_number) /**< number of arguments */
{
  ...
  QKING_ASSERT (ecma_is_value_empty (check_coercible_val));
                                            ...
                                            ecma_value_t regexp_arg) /**< routine's argument */
{
  ...
  if (ecma_is_value_empty (ret_value))
  {
    QKING_ASSERT (!ecma_is_value_empty (regexp_value));
    ...
    QKING_ASSERT (ecma_is_value_boolean (global_value));
    ...
    else
    {
      ...
      while (last_match && ecma_is_value_empty (ret_value))
      {
        ...
        else
        {
          ...
          {
            /* 8.f.iii.4. */
            QKING_ASSERT (ecma_is_value_object (exec_value));
...
{
  QKING_ASSERT (start <= end);
  QKING_ASSERT (end <= ecma_string_get_length (appended_string_p));
                                                                                                   ...
                                                                                                   * context */
{
  ...
  if (context_p->is_regexp)
  {
    ...
    {
      QKING_ASSERT (ecma_is_value_object (match_value));
      ...
      QKING_ASSERT (ecma_is_value_number (index_value));
      QKING_ASSERT (ecma_is_value_string (result_string_value));
      ...
      QKING_ASSERT ((ecma_length_t) ecma_number_to_uint32 (index_number) == context_p->match_start);
  ...
  {
    QKING_ASSERT (!context_p->is_global);
                                                         ...
                                                         ecma_value_t match_value) /**< returned match value */
{
  ...
  QKING_ASSERT (ecma_is_value_number (match_length_value));
  ...
  QKING_ASSERT ((ecma_length_t) ecma_number_to_uint32 (match_length_number) == match_length);
  QKING_ASSERT (match_length >= 1);
    ...
    while (replace_str_curr_p < replace_str_end_p)
    {
      ...
      if (action != LIT_CHAR_NULL)
      {
        ...
        else
        {
          ...
          QKING_ASSERT (action == LIT_CHAR_AMPERSAND || (action >= LIT_CHAR_0 && action <= LIT_CHAR_9));
          ...
          if (action >= LIT_CHAR_0 && action <= LIT_CHAR_9)
          {
            ...
            QKING_ASSERT (index > 0 && index < match_length);
          }
          ...
          {
            QKING_ASSERT (ecma_is_value_string (submatch_value));
                                                                                                  ...
                                                                                                  * context */
{
  ...
  while (continue_match)
  {
    ...
    if (!ecma_is_value_null (match_value))
    {
      ...
      QKING_ASSERT (ecma_is_value_string (string_value));
      ...
      {
        QKING_ASSERT (context_p->is_regexp);
                                             ...
                                             ecma_value_t regexp_arg) /**< routine's argument */
{
  ...
  if (ecma_is_value_empty (ret_value))
  {
    ...
    {
      QKING_ASSERT (ecma_is_value_object (match_result));
                                            ...
                                            ecma_value_t arg2) /**< routine's second argument */
{
  ...
  QKING_ASSERT (start <= len && end <= len);
                                            ...
                                            ecma_value_t arg2) /**< limit */
{
  ...
  if (ecma_is_value_empty (ret_value) && limit != 0)
  {
    ...
    if (ecma_is_value_undefined (arg1))
    {
      ...
      QKING_ASSERT (ecma_is_value_true (put_comp));
    ...
    else /* if (!ecma_is_value_undefined (arg1)) */
    {
      ...
      if (ecma_string_is_empty (this_to_string_p) && ecma_is_value_empty (ret_value))
      {
        ...
        if (separator_is_regexp)
        {
          ...
          {
            match_result = QKING_CONTEXT (error_value);
      ...
      else
      {
        ...
        while (curr_pos < string_length && !should_return && ecma_is_value_empty (ret_value))
        {
          ...
          else
          {
            ...
            QKING_ASSERT (!ECMA_IS_VALUE_ERROR (match_comp_value));
                                                                          ...
                                                                          *   or upper (false) case */
{
  ...
  while (input_str_curr_p < input_str_end_p)
  {
    ...
    QKING_ASSERT (character_length >= 1 && character_length <= LIT_MAXIMUM_OTHER_CASE_LENGTH);
  ...
  QKING_ASSERT (output_start_p + output_length == output_char_p);
                                             ...
                                             ecma_value_t length) /**< routine's second argument */
{
  ...
  if (ecma_is_value_empty (ret_value))
  {
    ...
    /* 5. */
    ecma_number_t from_num = (start_num < 0) ? QKING_MAX (this_len + start_num, 0) : start_num;
    uint32_t from = ecma_builtin_helper_string_index_normalize (from_num, ecma_number_to_uint32 (this_len), true);

    /* 6-7. */
    ecma_number_t to_num = QKING_MAX (QKING_MIN (QKING_MAX (length_num, 0), this_len - from_num), 0);
