...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_global.inc.h"
    ...
    if (string_curr_p < string_end_p)
    {
      ...
      if (ecma_is_value_empty (ret_value))
      {
        ...
        while (string_curr_p > start_p)
        {
          ...
          {
            QKING_ASSERT (lit_char_is_decimal_digit (current_char));
                                              ...
                                              const uint8_t *reserved_uri_bitset) /**< reserved characters bitset */
{
  ...
  QKING_ASSERT (ecma_is_value_string (string));
  ...
  if (ecma_is_value_empty (ret_value))
  {
    ...
    if (ecma_is_value_empty (ret_value))
    {
      QKING_ASSERT (output_start_p + output_size == output_char_p);
                                   ...
                                   ecma_value_t arg) /**< routine's first argument */
{
  ...
  while (input_curr_p < input_end_p)
  {
    ...
    else if (chr > ECMA_ESCAPE_MAXIMUM_BYTE_VALUE)
    {
      ...
      ecma_builtin_global_object_byte_to_hex (output_char_p + 1, (lit_utf8_byte_t) (chr >> QKING_BITSINBYTE));
                                     ...
                                     ecma_value_t arg) /**< routine's first argument */
{
  ...
  while (input_curr_p < input_end_p)
  {
    ...
    QKING_ASSERT (output_char_p <= input_curr_p);
  }
