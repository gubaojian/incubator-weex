...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_math.inc.h"
                                  ...
                                  ecma_length_t args_number) /**< number of arguments */
{
  ...
  while (args_number > 0)
  {
    ...
    if (QKING_UNLIKELY (ecma_number_is_nan (arg_num)))
    {
      result_num = arg_num;
      break;
    }
                                    ...
                                    ecma_length_t arguments_number) /**< length of arguments' list */
{
  ...
  QKING_ASSERT (builtin_routine_id == ECMA_MATH_OBJECT_RANDOM);
