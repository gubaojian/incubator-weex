...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_regexp_prototype.inc.h"
  ...
  else
  {
        ...
        && ecma_object_class_is (ecma_get_object_from_value (pattern_arg), LIT_MAGIC_STRING_REGEXP_UL))
    {
      ...
      else
      {
        ...
        QKING_ASSERT (ecma_is_value_boolean (ignore_case_value));
        ...
        QKING_ASSERT (ecma_is_value_boolean (multiline_value));
        ...
        QKING_ASSERT (ecma_is_value_empty (bc_comp));
