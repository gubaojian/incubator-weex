...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_typedarray_prototype.inc.h"
...
{
  QKING_ASSERT (mode < TYPEDARRAY_ROUTINE__COUNT);
  ...
  for (uint32_t index = 0; index < len && ecma_is_value_empty (ret_value); index++)
  {
    ...
    QKING_ASSERT (ecma_is_value_number (get_value));
                                                         ...
                                                         bool is_right) /**< choose order, true is reduceRight */
{
  ...
  if (ecma_is_value_undefined (initial_val))
  {
    ...
    QKING_ASSERT (ecma_is_value_number (accumulator));
                                          ...
                                          ecma_value_t cb_this_arg) /**< 'this' of the callback function */
{
  ...
  if (ecma_is_value_empty (ret_value))
  {
    ...
    if (!ECMA_IS_VALUE_ERROR (ret_value))
    {
      ...
      QKING_ASSERT (ecma_typedarray_get_offset (obj_p) == 0);
                                        ...
                                        ecma_value_t compare_func) /**< comparator fn */
{
  ...
  while (byte_index < limit)
  {
    QKING_ASSERT (buffer_index < typedarray_length);
