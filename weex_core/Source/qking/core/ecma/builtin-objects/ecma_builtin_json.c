...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_json.inc.h"
                            ...
                            const char *string_id_p) /**< string identifier */
{
   ...
   */
  QKING_ASSERT (*string_p == *string_id_p);
...
ecma_builtin_json_parse_value (ecma_json_token_t *token_p) /**< token argument */
{
  ...
  switch (token_p->type)
  {
    ...
    case left_square_token:
    {
      ...
      QKING_ASSERT (!ECMA_IS_VALUE_ERROR (array_construction));
...
{
  QKING_ASSERT (reviver_p);
  QKING_ASSERT (holder_p);
  QKING_ASSERT (name_p);
    ...
    while (ecma_value_p != NULL && ecma_is_value_empty (ret_value))
    {
      ...
      if (ecma_is_value_undefined (value_walk))
      {
        ecma_value_t delete_val = ecma_op_general_object_delete (object_p,
                                                                 property_name_p,
                                                                 false);
        QKING_ASSERT (ecma_is_value_boolean (delete_val));
      }
      else
      {
  ...
  {
    QKING_ASSERT (ECMA_IS_VALUE_ERROR (ret_value));
  }
...
ecma_builtin_json_quote (ecma_string_t *string_p) /**< string that should be quoted*/
{
  ...
  *buf++ = LIT_CHAR_DOUBLE_QUOTE;
  ...
  QKING_ASSERT ((size_t) (buf - buf_begin) == n_bytes);
                          ...
                          ecma_json_stringify_context_t *context_p) /**< context*/
{
  ...
  else
  {
    ...
    while (ecma_value_p != NULL)
    {
      ...
      QKING_ASSERT (ecma_is_property_enumerable (property));
  ...
  while (ecma_value_p != NULL && ecma_is_value_empty (ret_value))
  {
    ...
    if (!ecma_is_value_undefined (str_val))
    {
      ...
      QKING_ASSERT (!ECMA_IS_VALUE_ERROR (str_comp_val));
