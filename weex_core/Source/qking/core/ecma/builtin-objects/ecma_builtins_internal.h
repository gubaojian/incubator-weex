...
typedef enum {
  ...
  ECMA_BUILTIN_NUMBER_POSITIVE_INFINITY, /**< result of
                                            ecma_number_make_infinity (false) */
  ECMA_BUILTIN_NUMBER_NEGATIVE_INFINITY, /**< result of
...
#define BUILTIN_ROUTINE(builtin_id, object_type, object_prototype_builtin_id, \
  ...
  ecma_value_t ecma_builtin_##lowercase_name##_dispatch_call(                 \
      const ecma_value_t *, ecma_length_t);                                   \
  ecma_value_t ecma_builtin_##lowercase_name##_dispatch_construct(            \
      const ecma_value_t *, ecma_length_t);                                   \
  ecma_value_t ecma_builtin_##lowercase_name##_dispatch_routine(              \
      uint16_t builtin_routine_id, ecma_value_t this_arg_value,               \
      const ecma_value_t[], ecma_length_t);
