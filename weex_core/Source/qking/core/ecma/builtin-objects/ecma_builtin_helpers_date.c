 ...
 */
inline ecma_number_t QKING_ATTR_ALWAYS_INLINE
ecma_date_day (ecma_number_t time) /**< time value */
{
  QKING_ASSERT (!ecma_number_is_nan (time));
...
{
  QKING_ASSERT (!ecma_number_is_nan (year));
 ...
 */
static inline ecma_number_t QKING_ATTR_ALWAYS_INLINE
...
ecma_date_in_leap_year (ecma_number_t year) /**< time value */
{
  ...
  QKING_ASSERT (mod_400 >= -399 && mod_400 <= 399);
...
ecma_date_month_from_time (ecma_number_t time) /**< time value */
{
  ...
  QKING_ASSERT (day_within_year >= 0);
  ...
  QKING_ASSERT (day_within_year < 365);
...
{
  return qking_port_get_local_time_zone_adjustment (time, true);
...
{
  return time - qking_port_get_local_time_zone_adjustment (time, false);
} /* ecma_date_utc */
                            ...
                            const char *format_p) /**< format buffer */
{
  ...
  QKING_VLA (lit_utf8_byte_t, date_buffer, date_buffer_length);
  ...
  while (*format_p != LIT_CHAR_NULL)
  {
    ...
    switch (*format_p)
    {
      ...
      default:
      {
        QKING_ASSERT (*format_p == LIT_CHAR_UPPERCASE_Z); /* Time zone seconds part. */
  ...
  QKING_ASSERT (dest_p <= date_buffer + date_buffer_length);
