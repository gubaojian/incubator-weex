...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_promise.inc.h"
...
ecma_builtin_promise_reject_abrupt (ecma_value_t capability) /**< reject description */
{
  ...
  ecma_value_t reason = QKING_CONTEXT (error_value);
...
{
  QKING_ASSERT (ecma_is_value_object (capability)
                && ecma_is_value_object (array)
                && ecma_is_value_object (ctor));
  QKING_ASSERT (ecma_get_object_builtin_id (ecma_get_object_from_value (ctor)) == ECMA_BUILTIN_ID_PROMISE);
  QKING_ASSERT (ecma_get_object_type (ecma_get_object_from_value (array)) == ECMA_OBJECT_TYPE_ARRAY);
  ...
  QKING_ASSERT (!ecma_is_value_empty (ret));
...
{
  QKING_ASSERT (ecma_is_value_object (remaining));
  ...
  QKING_ASSERT (ext_object_p->u.class_prop.class_id == LIT_MAGIC_STRING_NUMBER_UL);

  QKING_ASSERT (ecma_is_value_integer_number (ext_object_p->u.class_prop.u.value));
                                  ...
                                  const ecma_length_t argc) /**< argument number */
{
  ...
  QKING_ASSERT (ecma_is_value_boolean (already_called));
  ...
  QKING_ASSERT (ecma_is_value_integer_number (index_val));
