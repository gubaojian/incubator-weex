...
#define ROUTINE_ARG_LIST_NON_FIXED                          \
  ROUTINE_ARG_LIST_0, const ecma_value_t *arguments_list_p, \
 ...
 */
enum {
  PASTE(ECMA_ROUTINE_START_, BUILTIN_UNDERSCORED_ID) =
      ECMA_BUILTIN_ID__COUNT - 1,
#define ROUTINE(name, c_function_name, args_number, length_prop_value) \
...
#ifndef BUILTIN_CUSTOM_DISPATCH
#define ROUTINE(name, c_function_name, args_number, length_prop_value)       \
  {name, ECMA_BUILTIN_PROPERTY_ROUTINE, ECMA_PROPERTY_CONFIGURABLE_WRITABLE, \
   ECMA_ROUTINE_VALUE(ECMA_ROUTINE_##name##c_function_name,                  \
                      length_prop_value)},
...
#define OBJECT_VALUE(name, obj_builtin_id, prop_attributes) \
  {name, ECMA_BUILTIN_PROPERTY_OBJECT, prop_attributes, obj_builtin_id},
#define SIMPLE_VALUE(name, simple_value, prop_attributes) \
  {name, ECMA_BUILTIN_PROPERTY_SIMPLE, prop_attributes, simple_value},
#define NUMBER_VALUE(name, number_value, prop_attributes) \
  {name, ECMA_BUILTIN_PROPERTY_NUMBER, prop_attributes, number_value},
#define STRING_VALUE(name, magic_string_id, prop_attributes) \
  {name, ECMA_BUILTIN_PROPERTY_STRING, prop_attributes, magic_string_id},
#define ACCESSOR_READ_WRITE(name, c_getter_name, c_setter_name,      \
                            prop_attributes)                         \
  {name, ECMA_BUILTIN_PROPERTY_ACCESSOR_READ_WRITE, prop_attributes, \
   ECMA_ACCESSOR_READ_WRITE(ECMA_ACCESSOR_##name##c_getter_name,     \
                            ECMA_ACCESSOR_##name##c_setter_name)},
#define ACCESSOR_READ_ONLY(name, c_getter_func_name, prop_attributes) \
  {name, ECMA_BUILTIN_PROPERTY_ACCESSOR_READ_ONLY, prop_attributes,   \
   ECMA_ACCESSOR_##name##c_getter_func_name},
#include BUILTIN_INC_HEADER_NAME
                                              ...
                                              arguments' list */
{
  ...
  QKING_UNUSED(this_arg_value);
  QKING_UNUSED(arguments_list);
  QKING_UNUSED(arguments_number);

  switch (builtin_routine_id) {
#define ROUTINE_ARG(n) \
  (arguments_number >= n ? arguments_list[n - 1] : ECMA_VALUE_UNDEFINED)
