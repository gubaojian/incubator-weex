...
#define BUILTIN_INC_HEADER_NAME "ecma_builtin_error_prototype.inc.h"
...
ecma_builtin_error_prototype_object_to_string (ecma_value_t this_arg) /**< this argument */
{
  ...
  else
  {
    ...
    if (QKING_UNLIKELY (ECMA_IS_VALUE_ERROR (name_to_str_completion)))
    {
    ...
    else
    {
      ...
      if (QKING_UNLIKELY (ECMA_IS_VALUE_ERROR (msg_to_str_completion)))
      {
      ...
      else
      {
        ...
        else
        {
          ...
          QKING_ASSERT (bytes == name_size);
          ret_str_buffer_p = ret_str_buffer_p + bytes;
          QKING_ASSERT (ret_str_buffer_p <= ret_str_buffer + size);
