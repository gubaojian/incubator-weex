 ...
 *      statement with same argument as corresponding ECMA_TRY_CATCH's first
 * argument.
 */
#define ECMA_TRY_CATCH(var, op, return_value)     \
  QKING_ASSERT(return_value == ECMA_VALUE_EMPTY); \
...
/**
 * The macro defines try-block that tries to perform ToNumber operation on given
 * value and checks for exceptions that might be thrown during the operation.
 *
 * If no exception was thrown, then code after the try-block is executed.
 * Otherwise, throw-completion value is just copied to return_value.
 *
 * Note:
 *      Each ECMA_OP_TO_NUMBER_TRY_CATCH should have it's own corresponding
 * ECMA_OP_TO_NUMBER_FINALIZE statement with same argument as corresponding
 * ECMA_OP_TO_NUMBER_TRY_CATCH's first argument.
 */
#define ECMA_OP_TO_NUMBER_TRY_CATCH(num_var, value, return_value) \
  ...
  return_value = ecma_get_number(value, &num_var);                \
                                                                  \
  if (QKING_LIKELY(ecma_is_value_empty(return_value))) {
/**
 * The macro marks end of code block that is defined by corresponding
 * ECMA_OP_TO_NUMBER_TRY_CATCH.
 *
 * Note:
 *      Each ECMA_OP_TO_NUMBER_TRY_CATCH should be followed by
 * ECMA_OP_TO_NUMBER_FINALIZE with same argument as corresponding
 * ECMA_OP_TO_NUMBER_TRY_CATCH's first argument.
 */
