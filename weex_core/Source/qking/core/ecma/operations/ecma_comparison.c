                                   ...
                                   ecma_value_t y) /**< second operand */
{
  ...
  if (ecma_is_value_number (x))
  {
    if (ecma_is_value_number (y))
    {
      ...
      QKING_ASSERT (is_x_equal_to_y == is_x_equal_to_y_check);
#endif /* !QKING_NDEBUG */
                                 ...
                                 ecma_value_t y) /**< second operand */
{
  ...
  QKING_ASSERT (ecma_is_value_number (x) || ecma_is_value_string (x));
  QKING_ASSERT (ecma_is_value_number (y) || ecma_is_value_string (y));
                                     ...
                                     bool left_first) /**< 'LeftFirst' flag */
{
  ...
  if (!(is_px_string && is_py_string))
  {
    ...
    else
    {
      ...
      {
        /* l. */
        QKING_ASSERT (!ecma_number_is_nan (nx)
                      && !ecma_number_is_infinity (nx));
        QKING_ASSERT (!ecma_number_is_nan (ny)
                      && !ecma_number_is_infinity (ny));
        QKING_ASSERT (!(ecma_number_is_zero (nx)
                        && ecma_number_is_zero (ny)));
      ...
      QKING_ASSERT (is_x_less_than_y_check == is_x_less_than_y);
#endif /* !QKING_NDEBUG */
  ...
  { /* 4. */
    QKING_ASSERT (is_px_string && is_py_string);
