 ...
 *         false - otherwise
 */
inline bool QKING_ATTR_ALWAYS_INLINE
                                ...
                                const ecma_compiled_code_t *bytecode_data_p) /**< byte-code array */
{
    ...
    if (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_STATIC_FUNCTION)
    {
    ...
    ext_func_p->u.function.closure_cp = ECMA_NULL_POINTER;
    /* 9. */
                                      ...
                                      ecma_value_t this_binding) /**< value of 'this' binding */
{
    ...
    arrow_func_p->closure_cp = ECMA_NULL_POINTER;
 ...
 */
inline const ecma_compiled_code_t * QKING_ATTR_ALWAYS_INLINE
...
{
    QKING_ASSERT (func_obj_p != NULL
                  && !ecma_is_lexical_environment (func_obj_p));
    
    while (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_BOUND_FUNCTION)
    {
        QKING_ASSERT (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_BOUND_FUNCTION);
    ...
    QKING_ASSERT (ecma_is_normal_or_arrow_function (ecma_get_object_type (func_obj_p))
    ...
    QKING_ASSERT (prototype_obj_p != NULL);
 ...
 */
static inline const ecma_value_t * QKING_ATTR_ALWAYS_INLINE
ecma_op_function_set_construct_flag (const ecma_value_t *arguments_list_p) /**< original arguments list pointer */
{
    ...
    QKING_ASSERT ((((uintptr_t) arguments_list_p) & 0x3) == 0);
    
#ifndef CONFIG_DISABLE_ES2015_CLASS
    arguments_list_p = (ecma_value_t *)(((uintptr_t) arguments_list_p) | ECMA_CLASS_CONSTRUCT_FLAG);
#endif /* !CONFIG_DISABLE_ES2015_CLASS */
...
{
    QKING_ASSERT (lex_env_p);
    QKING_ASSERT (ecma_op_resolve_super_reference_value (lex_env_p));
    QKING_ASSERT (ecma_get_lex_env_type (lex_env_p) != ECMA_LEXICAL_ENVIRONMENT_SUPER_OBJECT_BOUND);
    
    while (true)
    {
        ecma_object_t *lex_env_outer_p = ecma_get_lex_env_outer_reference (lex_env_p);
        
        QKING_ASSERT (lex_env_outer_p);
        
        if (ecma_get_lex_env_type (lex_env_outer_p) == ECMA_LEXICAL_ENVIRONMENT_SUPER_OBJECT_BOUND)
        {
            QKING_ASSERT (ecma_get_lex_env_type (lex_env_p) == ECMA_LEXICAL_ENVIRONMENT_DECLARATIVE);
            return lex_env_p;
        }
...
ecma_op_set_super_called (ecma_object_t *lex_env_p) /**< starting lexical enviroment */
{
    ...
    QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (*property_p) == ECMA_PROPERTY_TYPE_INTERNAL);
...
{
    QKING_ASSERT (ecma_is_value_object (this_binding));
    ecma_property_t *property_p = ecma_op_get_class_this_binding_property (lex_env_p);
    ...
    if (property_p)
    {
        QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (*property_p) == ECMA_PROPERTY_TYPE_NAMEDDATA);
...
ecma_op_get_class_this_binding (ecma_object_t *lex_env_p) /**< starting lexical enviroment */
{
    ...
    QKING_ASSERT (ecma_is_value_object (value_p->value));
...
{
    QKING_UNUSED_4 (function_obj, this_val, args_p, args_count);
...
{
    QKING_ASSERT (ecma_is_value_object (completion_value));
    QKING_ASSERT (ecma_is_value_object (this_arg));
                       ...
                       ecma_length_t arguments_list_len) /**< length of arguments list */
{
    QKING_ASSERT (func_obj_p != NULL
                  && !ecma_is_lexical_environment (func_obj_p));
    QKING_ASSERT (ecma_op_is_callable (ecma_make_object_value (func_obj_p)));
    
    while (true)
    {
        ecma_object_type_t func_type = ecma_get_object_type (func_obj_p);
        
        QKING_ASSERT (func_type == ECMA_OBJECT_TYPE_FUNCTION
        ...
        {
            if (QKING_UNLIKELY (ecma_get_object_is_builtin (func_obj_p)))
            {
                QKING_ASSERT (!ecma_op_function_has_construct_flag (arguments_list_p));
...
#ifndef CONFIG_DISABLE_ES2015_CLASS
            bool is_class_constructor = (bool) (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_CONSTRUCTOR);
            
            if (is_class_constructor)
            {
                if (arguments_list_len > 0 && !ecma_op_function_has_construct_flag (arguments_list_p)) {
                    return ecma_raise_type_error (ECMA_ERR_MSG ("Class constructor cannot be invoked without 'new'."));
                }
            }
#endif /* !CONFIG_DISABLE_ES2015_CLASS */
            
            bool is_strict = (bool) (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_STRICT_MODE);
            bool is_no_lex_env = (bool) (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_LEXICAL_ENV_NOT_NEEDED);
            ...
            if (!is_strict)
            {
                ...
                else if (!ecma_is_value_object (this_binding))
                {
                    ...
                    QKING_ASSERT (!ECMA_IS_VALUE_ERROR (this_binding));
                }
            }
            if (arguments_list_len > 0) {
                arguments_list_p = ecma_op_function_clear_construct_flag ((ecma_value_t *)arguments_list_p);
            }
            ...
            else
            {
                ...
                if (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_ARGUMENTS_NEEDED)
                {
...
#ifndef CONFIG_DISABLE_ES2015_CLASS
                if (QKING_UNLIKELY (is_class_constructor))
                {
                    //todo old jerry has this problem. find out if comment this line has any side affect
//                    ecma_op_set_class_this_binding (local_env_p, this_binding);
            ...
            if (QKING_UNLIKELY (free_this_binding))
            {
        ...
        else if (func_type == ECMA_OBJECT_TYPE_EXTERNAL_FUNCTION)
        {
            ...
            if (QKING_UNLIKELY (ecma_is_value_error_reference (ret_value)))
            {
                QKING_CONTEXT (error_value) = ecma_clear_error_reference (ret_value, true);
                return ECMA_VALUE_ERROR;
            }
            
#ifdef QKING_DEBUGGER
            QKING_DEBUGGER_CLEAR_FLAGS (QKING_DEBUGGER_VM_EXCEPTION_THROWN);
        ...
        else if (func_type == ECMA_OBJECT_TYPE_ARROW_FUNCTION)
        {
            ...
            is_no_lex_env = (bool) (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_LEXICAL_ENV_NOT_NEEDED);
            ...
            if (!is_no_lex_env)
            {
                ...
                QKING_ASSERT (!(bytecode_data_p->status_flags & ECMA_CODE_FLAGS_ARGUMENTS_NEEDED));
            }
        ...
        {
#ifndef CONFIG_DISABLE_ES2015_CLASS
            if (QKING_UNLIKELY (args_len_or_this == ECMA_VALUE_IMPLICIT_CONSTRUCTOR))
            {
                            ...
                            ecma_length_t arguments_list_len) /**< length of arguments list */
{
    ...
    QKING_ASSERT (ecma_is_value_object (this_arg_value)
                  || this_arg_value == ECMA_VALUE_UNDEFINED);
    
    ecma_object_t *target_func_obj_p = NULL;
    
    while (QKING_UNLIKELY (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_BOUND_FUNCTION))
    {
        /* 1-3. */
        ecma_extended_object_t *ext_function_p = (ecma_extended_object_t *) func_obj_p;
...
#ifndef CONFIG_DISABLE_ES2015_ARROW_FUNCTION
    if (QKING_UNLIKELY (type == ECMA_OBJECT_TYPE_ARROW_FUNCTION))
    {
        return ecma_raise_type_error (ECMA_ERR_MSG ("Arrow functions have no constructor."));
    }
#endif /* CONFIG_DISABLE_ES2015_ARROW_FUNCTION */
    
    if (QKING_UNLIKELY (type == ECMA_OBJECT_TYPE_FUNCTION && ecma_get_object_is_builtin (func_obj_p)))
    {
    ...
    if (QKING_LIKELY (this_arg_value == ECMA_VALUE_UNDEFINED))
    {
        /* 5. */
    ...
    switch (type)
    {
        ...
        {
            QKING_ASSERT (!ecma_op_function_has_construct_flag (arguments_list_p));
            QKING_ASSERT (target_func_obj_p != NULL);
        ...
        default:
        {
            QKING_ASSERT (type == ECMA_OBJECT_TYPE_EXTERNAL_FUNCTION);
    ...
    if (QKING_UNLIKELY (new_this_obj_p == NULL))
    {
...
{
    QKING_ASSERT (!ecma_get_object_is_builtin (object_p));
        ...
        || ecma_compare_ecma_string_to_magic_id (property_name_p, LIT_MAGIC_STRING_ARGUMENTS))
    {
        ...
        if (bytecode_data_p->status_flags & ECMA_CODE_FLAGS_STRICT_MODE)
        {
...
{
    QKING_ASSERT (ecma_get_object_type (object_p) == ECMA_OBJECT_TYPE_EXTERNAL_FUNCTION);
...
ecma_op_bound_function_try_to_lazy_instantiate_property (ecma_object_t *object_p, /**< object */
                                                         ecma_string_t *property_name_p) /**< property's name */
{
    QKING_ASSERT (ecma_get_object_type (object_p) == ECMA_OBJECT_TYPE_BOUND_FUNCTION);
    ...
    if (ecma_string_is_length (property_name_p))
    {
        ...
        if (ecma_object_get_class_name (target_func_obj_p) == LIT_MAGIC_STRING_FUNCTION_UL)
        {
            ...
            QKING_ASSERT (!ECMA_IS_VALUE_ERROR (get_len_value));
            QKING_ASSERT (ecma_is_value_integer_number (get_len_value));
...
ecma_op_function_list_lazy_property_names (ecma_object_t *object_p, /**< functionobject */
                                           ...
                                           ecma_collection_header_t *non_enum_collection_p) /**< skipped
                                                                                             ...
                                                                                             *   collection */
{
    QKING_UNUSED (main_collection_p);
...
void ecma_op_function_remove_register_out_closure(ecma_register_t *register_p) {
    QKING_ASSERT(register_p->next_p);
    ecma_register_t *prev_p = NULL;
    ecma_register_t *next_p = NULL;
    if (register_p->prev_p) {
        prev_p = register_p->prev_p;
    }
    if (register_p->next_p) {
        next_p = register_p->next_p;
    }
    ecma_register_t *out_closure_p = next_p;
    ecma_fast_free_value(out_closure_p->var);
    if (prev_p) {
        prev_p->next_p = out_closure_p->next_p;
        out_closure_p->next_p->prev_p = prev_p;
    }
    else {
        if (out_closure_p->next_p) {
            out_closure_p->next_p->prev_p = NULL;
        }
    }
    jmem_heap_free_block(out_closure_p, sizeof(ecma_register_t));
    register_p->prev_p = NULL;
    register_p->next_p = NULL;
}

void ecma_op_function_remove_register_in_closure(ecma_register_t *register_p) {
    ecma_register_t *prev_p = NULL;
    ecma_register_t *next_p = NULL;
    if (register_p->prev_p) {
        prev_p = register_p->prev_p;
...
void ecma_op_function_add_out_closure(ecma_register_t *register_p) {
    ecma_register_t *register_ref_p = jmem_heap_alloc_block_null_on_error(sizeof(ecma_register_t));
    if (register_ref_p) {
        if (register_p->next_p == NULL) {
            register_p->next_p = register_ref_p;
            register_ref_p->var = ecma_fast_copy_value(register_p->var);
            register_ref_p->prev_p = register_p;
            register_ref_p->next_p = NULL;
        }
        else {
            ecma_register_t *next_p = register_p->next_p;
            register_p->next_p = register_ref_p;
            register_ref_p->var = ecma_fast_copy_value(register_p->var);
            register_ref_p->prev_p = register_p;
            register_ref_p->next_p = next_p;
            next_p->prev_p = register_ref_p;
        }
    }
}

ecma_register_t *ecma_op_function_find_in_closure_register(vm_frame_ctx_t *frame_ctx_p, ecma_func_in_closure_t *func_in_closure_p)
{
    ...
    do {
        ...
        ecma_object_t *object_p = ecma_get_object_from_value(this_function);
        ...
        if (bytecode_data_p == (ecma_compiled_code_t *)func_in_closure_p->out_closure_p->func_state_p) {
            closure_ctx_p = frame_ctx_current_p;
            break;
        }
        frame_ctx_current_p = frame_ctx_current_p->prev_context_p;
        
    } while (true);
    
    QKING_ASSERT(closure_ctx_p);
    return &closure_ctx_p->registers_p[func_in_closure_p->out_closure_p->idx];
}

void ecma_op_function_add_to_register_list_end(ecma_register_t *register_list_p, ecma_register_t *register_p) {
    ecma_register_t *register_current_p = register_list_p;
    while (register_current_p->next_p) {
        register_current_p = register_current_p->next_p;
    }
    register_current_p->next_p = register_p;
    register_p->prev_p = register_current_p;
    register_p->next_p = NULL;
    register_p->var = ecma_fast_copy_value(register_current_p->var);
}

void ecma_op_function_free_closure(ecma_object_t *func_obj_p) {
    ecma_register_t **register_list_pp = NULL;
    const ecma_compiled_code_t *bytecode_data_p = NULL;
    if (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_ARROW_FUNCTION)
    {
        ecma_arrow_function_t *arrow_func_p = (ecma_arrow_function_t *) func_obj_p;
        register_list_pp = JMEM_CP_GET_POINTER(ecma_register_t *, arrow_func_p->closure_cp);
        bytecode_data_p = ecma_op_arrow_function_get_compiled_code (arrow_func_p);
    }
    else {
        ecma_extended_object_t *ext_func_p = (ecma_extended_object_t *) func_obj_p;
        register_list_pp = JMEM_CP_GET_POINTER(ecma_register_t *, ext_func_p->u.function.closure_cp);
        bytecode_data_p = ecma_op_function_get_compiled_code (ext_func_p);
    }
    ...
    for (int i = 0; i < in_closure; i++) {
        ecma_register_t *register_p = register_list_pp[i];
        if (register_p->next_p || register_p->prev_p) {
#ifdef QKING_ENABLE_GC_DEBUG
            printf("[gc][%d]=>[remove][in_closure][index][%d]\n", register_p->var, i);
#endif
            ecma_op_function_remove_register_in_closure(register_p);
        }
        ecma_fast_free_value(register_p->var);
        jmem_heap_free_block(register_p, sizeof(ecma_register_t));
    }
    jmem_heap_free_block(register_list_pp, sizeof(ecma_register_t *) * in_closure);
}

void ecma_op_function_load_in_closure(vm_frame_ctx_t *frame_ctx_p, ecma_object_t *func_obj_p, ecma_compiled_function_state_t *func_state_p) {
    ecma_func_in_closure_t **func_in_closure_pp = func_state_p->pp_in_closure;
    ecma_register_t **register_list_pp = NULL;
    if (func_state_p->in_closure > 0) {
        size_t size = sizeof(ecma_register_t *) * func_state_p->in_closure;
        register_list_pp = jmem_heap_alloc_block_null_on_error(size);
        memset(register_list_pp, 0, size);
    }
    for (int i = 0; i < func_state_p->in_closure; i++) {
        ecma_func_in_closure_t *func_in_closure_p = func_in_closure_pp[i];
        ecma_register_t *register_finder_p = ecma_op_function_find_in_closure_register(frame_ctx_p, func_in_closure_p);
        QKING_ASSERT(register_finder_p);
        if (!register_finder_p->next_p) {
#ifdef QKING_ENABLE_GC_DEBUG
            printf("[gc][%d]=>[add_out_closure][register][%d]\n", register_finder_p->var, (int)func_in_closure_p->out_closure_p->idx);
#endif
            ecma_op_function_add_out_closure(register_finder_p);
        }
#ifdef QKING_ENABLE_GC_DEBUG
        printf("[gc][%d][bind][func][%d]\n", register_finder_p->var, ecma_make_object_value(func_obj_p));
#endif
        ecma_register_t *register_p = jmem_heap_alloc_block_null_on_error(sizeof(ecma_register_t));
        if (register_p) {
#ifdef QKING_ENABLE_GC_DEBUG
            printf("[gc][%d][add_in_closure][register][%d]\n", register_finder_p->var, (int)func_in_closure_p->idx);
#endif
            ecma_op_function_add_to_register_list_end(register_finder_p, register_p);
            register_list_pp[i] = register_p;
        }
    }
    jmem_cpointer_t closure_cp;
    JMEM_CP_SET_NON_NULL_POINTER (closure_cp, register_list_pp);
    if (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_ARROW_FUNCTION)
    {
        ecma_arrow_function_t *arrow_func_p = (ecma_arrow_function_t *) func_obj_p;
        QKING_ASSERT(arrow_func_p->closure_cp == ECMA_NULL_POINTER);
        if (arrow_func_p->closure_cp != ECMA_NULL_POINTER) {
            ecma_op_function_free_closure(func_obj_p);
        }
        arrow_func_p->closure_cp = closure_cp;
    }
    else {
        ecma_extended_object_t *ext_func_p = (ecma_extended_object_t *) func_obj_p;
        QKING_ASSERT(ext_func_p->u.function.closure_cp == ECMA_NULL_POINTER);
#ifdef QKING_ENABLE_GC_DEBUG
        printf("[gc][%d]=>[set][closure_cp][%d]\n", ecma_make_object_value(func_obj_p), closure_cp);
#endif
        ext_func_p->u.function.closure_cp = closure_cp;
    }
}

void ecma_op_function_load_register_in_closure(vm_frame_ctx_t *frame_ctx_p, const ecma_compiled_function_state_t *func_state_p)
{
    ecma_object_t *func_obj_p = ecma_get_object_from_value(frame_ctx_p->this_function);
    ecma_func_in_closure_t **func_in_closure_pp = func_state_p->pp_in_closure;
    ecma_register_t **register_list_pp = NULL;
    if (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_ARROW_FUNCTION)
    {
    ...
    QKING_ASSERT(register_list_pp);
    for (int i = 0; i < func_state_p->in_closure; i++) {
        ecma_register_t *register_list_p = register_list_pp[i];
        ecma_register_t *register_p = frame_ctx_p->registers_p + func_in_closure_pp[i]->idx;
#ifdef QKING_ENABLE_GC_DEBUG
        printf("[gc][%d]=>[load][in_closure][%d]\n", register_p->var, (int)func_in_closure_pp[i]->idx);
#endif
        ecma_op_function_add_to_register_list_end(register_list_p, register_p);
    }
}
