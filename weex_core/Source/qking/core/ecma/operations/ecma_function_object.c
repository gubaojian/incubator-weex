...
//                    ecma_op_set_class_this_binding (local_env_p, this_binding);
        ...
        else if (func_type == ECMA_OBJECT_TYPE_EXTERNAL_FUNCTION)
        {
            ...
            {
                ecma_value_t error_value = ecma_clear_error_reference (ret_value, true);
                if (QKING_CONTEXT (error_value) != error_value) {
                    ecma_free_value(QKING_CONTEXT (error_value));
                    QKING_CONTEXT (error_value) = error_value;
                }
...
void ecma_ref_closure (ecma_closure_t *closure_p) /**< closure descriptor */
{
    QKING_ASSERT (closure_p != NULL);
    
    QKING_ASSERT (closure_p->refs_and_container >= ECMA_CLOSURE_REF_ONE);
    
    if (QKING_LIKELY (closure_p->refs_and_container < ECMA_CLOSURE_MAX_REF))
    {
        /* Increase reference counter. */
        closure_p->refs_and_container = (uint16_t) (closure_p->refs_and_container + ECMA_CLOSURE_REF_ONE);
    }
    else
    {
...
void ecma_deref_closure (ecma_closure_t *closure_p) /**< ecma-string */
{
    QKING_ASSERT (closure_p != NULL);
    ...
    if (closure_p->refs_and_container >= ECMA_CLOSURE_REF_ONE)
    {
        return;
    }
    ecma_free_value_if_not_object(closure_p->var);
    ecma_dealloc_closure(closure_p);
}

bool ecma_op_closure_add_register(ecma_closure_t *closure_p, ecma_register_t *register_p) {
    bool finished = true;
    do {
        ecma_register_t *register_list_p = closure_p->register_p;
        if (!register_list_p) {
            closure_p->register_p = register_p;
        }
        else {
            while (register_list_p) {
                if (register_list_p == register_p) {
                    finished = false;
                    break;
                }
                if (!register_list_p->next_p) {
                    break;
                }
                register_list_p = register_list_p->next_p;
            }
            if (!finished) {
                break;
            }
            QKING_ASSERT(register_list_p->next_p == NULL);
            register_list_p->next_p = register_p;
        }
        register_p->closure_p = closure_p;
...
void ecma_op_closure_remove_register(ecma_closure_t *closure_p, ecma_register_t *register_p) {
    ecma_register_t *register_list_p = closure_p->register_p;
    ecma_register_t *register_find_p = NULL;
    ecma_register_t *register_prev_p = NULL;
    while (register_list_p) {
        if (register_list_p == register_p) {
            register_find_p = register_p;
            break;
        }
        if (!register_list_p->next_p) {
            break;
        }
        register_prev_p = register_list_p;
        register_list_p = register_list_p->next_p;
    }
    QKING_ASSERT(register_find_p);
    if (!register_prev_p) {
        closure_p->register_p = register_p->next_p;
    }
    else {
        register_prev_p->next_p = register_p->next_p;
    }
    register_p->next_p = NULL;
    register_p->closure_p = NULL;
    ecma_deref_closure(closure_p);
}

void ecma_op_function_remove_register_from_closure(ecma_register_t *register_p) {
    QKING_ASSERT(register_p->closure_p);
    ecma_op_closure_remove_register(register_p->closure_p, register_p);
}

void ecma_op_function_add_out_closure(ecma_register_t *register_p) {
    ecma_closure_t *closure_p = register_p->closure_p;
    if (!closure_p) {
        closure_p = ecma_alloc_closure();
        closure_p->var = ECMA_VALUE_UNDEFINED;
        closure_p->refs_and_container = ECMA_CLOSURE_REF_ONE;
        closure_p->register_p = NULL;
        ecma_op_closure_add_register(closure_p, register_p);
        /** weak reference copy **/
        ecma_free_value_if_not_object(closure_p->var);
...
void ecma_op_function_free_closure(ecma_object_t *func_obj_p) {
    ...
    jmem_heap_free_block(closure_list_pp, sizeof(ecma_closure_t *) * in_closure);
}
...
void ecma_op_function_load_in_closure(vm_frame_ctx_t *frame_ctx_p, ecma_object_t *func_obj_p, ecma_compiled_function_state_t *func_state_p) {
    ...
    if (func_state_p->in_closure > 0) {
        size_t size = sizeof(ecma_closure_t *) * func_state_p->in_closure;
        closure_list_pp = jmem_heap_alloc_block_null_on_error(size);
        memset(closure_list_pp, 0, size);
    }
    for (int i = 0; i < func_state_p->in_closure; i++) {
        ...
        if (!register_finder_p->closure_p) {
#ifdef QKING_ENABLE_GC_DEBUG
...
#endif
        closure_list_pp[i] = register_finder_p->closure_p;
        ecma_ref_closure(register_finder_p->closure_p);
#ifdef QKING_ENABLE_GC_DEBUG
        printf("[gc][%d][add_in_closure][register][%d]\n", register_finder_p->var, (int)func_in_closure_p->idx);
#endif
    }
    jmem_cpointer_t closure_cp;
    JMEM_CP_SET_NON_NULL_POINTER (closure_cp, closure_list_pp);
    if (ecma_get_object_type (func_obj_p) == ECMA_OBJECT_TYPE_ARROW_FUNCTION)
    {
        ecma_arrow_function_t *arrow_func_p = (ecma_arrow_function_t *) func_obj_p;
...
void ecma_op_function_load_register_in_closure(vm_frame_ctx_t *frame_ctx_p, const ecma_compiled_function_state_t *func_state_p)
{
    ...
    QKING_ASSERT(closure_list_pp);
    for (int i = 0; i < func_state_p->in_closure; i++) {
        ecma_closure_t *closure_p = closure_list_pp[i];
        QKING_ASSERT(closure_p);
        ...
        register_p->var = ecma_fast_copy_value(closure_p->var);
    }
}
