...
QKING_STATIC_ASSERT (ECMA_MAP_OBJECT_ITEM_COUNT == 3,
...
bool ecma_op_is_map(ecma_value_t this_arg) {
    bool is_map = false;
    if (ecma_is_value_object (this_arg))
    {
        ecma_map_object_t *map_object_p = (ecma_map_object_t *) ecma_get_object_from_value (this_arg);
                         ...
                         ecma_value_t key) /**< key to search */
{
  ...
  while (true)
  {
    ...
    if (QKING_UNLIKELY (item == ECMA_VALUE_ARRAY_HOLE))
    {
      QKING_ASSERT (last_key == ECMA_VALUE_ARRAY_HOLE);
      continue;
    }

    if (QKING_UNLIKELY (ecma_is_value_pointer (item)))
    {
      ...
      QKING_ASSERT (!ecma_is_value_pointer (*item_p));
      continue;
    }
...
ecma_value_t *ecma_op_map_set_ref (ecma_value_t this_arg, ecma_value_t key_arg) {
    ecma_map_object_t *map_object_p = ecma_op_map_get_object (this_arg);
    if (map_object_p == NULL)
    {
        return NULL;
...
{
  QKING_ASSERT (ecma_get_object_type (&map_object_p->header.object) == ECMA_OBJECT_TYPE_CLASS
                && (map_object_p->header.u.class_prop.class_id == LIT_MAGIC_STRING_MAP_UL));
                          ...
                          ecma_map_object_chunk_t *prev_chunk_p) /**< previous chunk */
{
  for (int i = 0; i < ECMA_MAP_OBJECT_ITEM_COUNT; i++)
  {
    QKING_ASSERT (!ecma_is_value_pointer (chunk_p->items[i]));
  ...
  if (prev_chunk_p != NULL)
  {
    ...
    {
      QKING_ASSERT (map_object_p->first_chunk_cp != map_object_p->last_chunk_cp);
      QKING_ASSERT (ECMA_GET_NON_NULL_POINTER (ecma_map_object_chunk_t, map_object_p->last_chunk_cp) == chunk_p);
                    ...
                    ecma_value_t key_arg) /**< key argument */
{
  ...
  if (QKING_UNLIKELY (map_object_p->first_chunk_cp == ECMA_NULL_POINTER))
  {
  ...
  {
    QKING_ASSERT (!ecma_is_value_pointer (item_p[1]));
