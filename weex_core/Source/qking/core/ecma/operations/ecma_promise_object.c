...
{
  QKING_ASSERT (ecma_is_promise (obj_p));
                         ...
                         ecma_value_t result) /**< the result value */
{
  ...
  QKING_ASSERT (ext_object_p->u.class_prop.u.value == ECMA_VALUE_UNDEFINED);
 ...
 */
static inline uint8_t QKING_ATTR_ALWAYS_INLINE
...
{
  QKING_ASSERT (ecma_is_value_object (already_resolved));
  ...
  QKING_ASSERT (ext_object_p->u.class_prop.class_id == LIT_MAGIC_STRING_BOOLEAN_UL);
                     ...
                     ecma_value_t reason) /**< reason for reject */
{
  ...
  QKING_ASSERT (ecma_promise_get_state (obj_p) == ECMA_PROMISE_STATE_PENDING);
                              ...
                              const ecma_length_t argc) /**< argument number */
{
  ...
  {
    /* 9. */
    then = QKING_CONTEXT (error_value);
                               ...
                               ecma_promise_executor_type_t type) /**< indicates the type of executor */
{
  ...
  {
    QKING_ASSERT (ecma_op_is_callable (executor));
  ...
  {
    QKING_ASSERT (ecma_is_value_object (executor));
  ...
  {
    QKING_ASSERT (type == ECMA_PROMISE_EXECUTOR_EMPTY);
    QKING_UNUSED (executor);
  }
  ...
  {
    /* 10.a. */
    completion = QKING_CONTEXT (error_value);
