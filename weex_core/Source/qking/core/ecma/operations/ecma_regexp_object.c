 ...
 */
inline ecma_char_t QKING_ATTR_ALWAYS_INLINE
                 ...
                 const lit_utf8_byte_t **out_str_p) /**< [out] matching substring iterator */
{
  ...
  while (true)
  {
    ...
    switch (op)
    {
      case RE_OP_MATCH:
      {
        QKING_TRACE_MSG ("Execute RE_OP_MATCH: match\n");
      ...
      case RE_OP_CHAR:
      {
        ...
        QKING_TRACE_MSG ("Character matching %d to %d: ", ch1, ch2);

        if (ch1 != ch2)
        {
          QKING_TRACE_MSG ("fail\n");
          return ECMA_VALUE_FALSE; /* fail */
        }

        QKING_TRACE_MSG ("match\n");
      ...
      case RE_OP_PERIOD:
      {
        ...
        QKING_TRACE_MSG ("Period matching '.' to %u: ", (unsigned int) ch);
      ...
      {
        QKING_TRACE_MSG ("Execute RE_OP_ASSERT_START: ");
      ...
      {
        QKING_TRACE_MSG ("Execute RE_OP_ASSERT_END: ");
      ...
      case RE_OP_ASSERT_NOT_WORD_BOUNDARY:
      {
        ...
        {
          QKING_TRACE_MSG ("Execute RE_OP_ASSERT_WORD_BOUNDARY: ");
        ...
        {
          QKING_ASSERT (op == RE_OP_ASSERT_NOT_WORD_BOUNDARY);
          QKING_TRACE_MSG ("Execute RE_OP_ASSERT_NOT_WORD_BOUNDARY: ");
      ...
      case RE_OP_LOOKAHEAD_NEG:
      {
        ...
        if (!ECMA_IS_VALUE_ERROR (match_value))
        {
          QKING_TRACE_MSG ("Execute RE_OP_LOOKAHEAD_POS/NEG: ");
        ...
        if (!ECMA_IS_VALUE_ERROR (match_value))
        {
          ...
          {
            QKING_ASSERT (ecma_is_value_boolean (match_value));
            /* restore saved */
      ...
      case RE_OP_INV_CHAR_CLASS:
      {
        ...
        QKING_TRACE_MSG ("Execute RE_OP_CHAR_CLASS/RE_OP_INV_CHAR_CLASS, ");
        if (str_curr_p >= re_ctx_p->input_end_p)
        {
          QKING_TRACE_MSG ("fail\n");
          return ECMA_VALUE_FALSE; /* fail */
        ...
        while (num_of_ranges)
        {
          ...
          QKING_TRACE_MSG ("num_of_ranges=%u, ch1=%u, ch2=%u, curr_ch=%u; ",
        ...
        {
          QKING_ASSERT (op == RE_OP_INV_CHAR_CLASS);
          if (is_match)
          {
            QKING_TRACE_MSG ("fail\n");
            return ECMA_VALUE_FALSE; /* fail */
      ...
      case RE_OP_BACKREFERENCE:
      {
        ...
        QKING_TRACE_MSG ("Execute RE_OP_BACKREFERENCE (idx: %u): ", (unsigned int) backref_idx);
        backref_idx *= 2;  /* backref n -> saved indices [n*2, n*2+1] */
        QKING_ASSERT (backref_idx >= 2 && backref_idx + 1 < re_ctx_p->num_of_captures);
      ...
      case RE_OP_SAVE_AT_START:
      {
        ...
        QKING_TRACE_MSG ("Execute RE_OP_SAVE_AT_START\n");
      ...
      {
        QKING_TRACE_MSG ("End of pattern is reached: match\n");
      ...
      case RE_OP_ALTERNATIVE:
      {
        ...
        */
        uint32_t offset = re_get_value (&bc_p);
        QKING_TRACE_MSG ("Execute RE_OP_ALTERNATIVE");
        bc_p += offset;

        while (*bc_p == RE_OP_ALTERNATIVE)
        {
          QKING_TRACE_MSG (", jump: %u", (unsigned int) offset);
          bc_p++;
      ...
      case RE_OP_NON_CAPTURE_NON_GREEDY_ZERO_GROUP_START:
      {
        ...
        */
        ...
        {
          QKING_ASSERT (start_idx <= re_ctx_p->num_of_captures / 2);
        ...
        {
          QKING_ASSERT (start_idx < re_ctx_p->num_of_non_captures);
      ...
      case RE_OP_NON_CAPTURE_NON_GREEDY_GROUP_END:
      {
        ...
        */
        ...
        if (RE_IS_CAPTURE_GROUP (op))
        {
          QKING_ASSERT (end_idx <= re_ctx_p->num_of_captures / 2);
          iter_idx = end_idx - 1;
          end_idx = (end_idx * 2) + 1;
        }
        else
        {
          QKING_ASSERT (end_idx <= re_ctx_p->num_of_non_captures);
      ...
      case RE_OP_NON_GREEDY_ITERATOR:
      {
        ...
        offset = re_get_value (&bc_p);
        QKING_TRACE_MSG ("Non-greedy iterator, min=%lu, max=%lu, offset=%ld\n",
      ...
      default:
      {
        QKING_ASSERT (op == RE_OP_GREEDY_ITERATOR);
        ...
        offset = re_get_value (&bc_p);
        QKING_TRACE_MSG ("Greedy iterator, min=%lu, max=%lu, offset=%ld\n",
                         (unsigned long) min, (unsigned long) max, (long) offset);
                         ...
                         bool ignore_global) /**< ignore global flag */
{
  ...
  QKING_ASSERT (ecma_is_value_object (regexp_value));
  QKING_ASSERT (ecma_is_value_string (input_string));

  ecma_object_t *regexp_object_p = ecma_get_object_from_value (regexp_value);

  QKING_ASSERT (ecma_object_class_is (regexp_object_p, LIT_MAGIC_STRING_REGEXP_UL));
  ...
  QKING_TRACE_MSG ("Exec with flags [global: %d, ignoreCase: %d, multiline: %d]\n",
  ...
  QKING_ASSERT (re_ctx.num_of_captures % 2 == 0);
  ...
  if (ecma_is_value_empty (ret_value))
  {
    if (is_match)
    {
      ...
      for (uint32_t i = 0; i < re_ctx.num_of_captures; i += 2)
      {
        ...
        QKING_ASSERT (!ecma_is_value_object (capture_value));
