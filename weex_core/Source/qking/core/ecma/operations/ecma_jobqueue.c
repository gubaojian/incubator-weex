...
{
  QKING_CONTEXT (job_queue_head_p) = NULL;
  QKING_CONTEXT (job_queue_tail_p) = NULL;
} /* ecma_job_queue_init */
...
ecma_create_promise_reaction_job (ecma_value_t reaction, /**< PromiseReaction */
                                  ecma_value_t argument) /**< argument for the reaction */
{
  QKING_ASSERT (ecma_is_value_object (reaction));
...
ecma_create_promise_resolve_thenable_job (ecma_value_t promise, /**< promise to be resolved */
                                          ...
                                          ecma_value_t then) /**< 'then' function */
{
  QKING_ASSERT (ecma_is_promise (ecma_get_object_from_value (promise)));
  QKING_ASSERT (ecma_is_value_object (thenable));
  QKING_ASSERT (ecma_op_is_callable (then));
...
ecma_process_promise_reaction_job (void *obj_p) /**< the job to be operated */
{
  ...
  QKING_ASSERT (ecma_is_value_boolean (handler) || ecma_op_is_callable (handler));
  ...
  else
  {
    ...
    QKING_ASSERT (ecma_op_is_callable (resolve));
...
ecma_process_promise_resolve_thenable_job (void *obj_p) /**< the job to be operated */
{
  ...
  {
    then_call_result = QKING_CONTEXT (error_value);
                  ...
                  void *job_p) /**< the job */
{
  ...
  if (QKING_CONTEXT (job_queue_head_p) == NULL)
  {
    QKING_CONTEXT (job_queue_head_p) = item_p;
    QKING_CONTEXT (job_queue_tail_p) = item_p;
  }
  else
  {
    QKING_CONTEXT (job_queue_tail_p)->next_p = item_p;
    QKING_CONTEXT (job_queue_tail_p) = item_p;
  }
} /* ecma_enqueue_job */
...
ecma_process_all_enqueued_jobs (void)
{
  ...
  while (QKING_CONTEXT (job_queue_head_p) != NULL && !ECMA_IS_VALUE_ERROR (ret))
  {
    ecma_job_queueitem_t *item_p = QKING_CONTEXT (job_queue_head_p);
    QKING_CONTEXT (job_queue_head_p) = QKING_CONTEXT (job_queue_head_p)->next_p;
...
{
  while (QKING_CONTEXT (job_queue_head_p) != NULL)
  {
    ecma_job_queueitem_t *item_p = QKING_CONTEXT (job_queue_head_p);
    QKING_CONTEXT (job_queue_head_p) = item_p->next_p;
    void *job_p = item_p->job_p;
    jmem_heap_free_block (item_p, sizeof (ecma_job_queueitem_t));
