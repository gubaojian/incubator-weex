 ...
 */
#ifndef QKING_NDEBUG
#define QKING_ASSERT_OBJECT_TYPE_IS_VALID(type) \
  QKING_ASSERT (type < ECMA_OBJECT_TYPE__MAX);
#else /* QKING_NDEBUG */
#define QKING_ASSERT_OBJECT_TYPE_IS_VALID(type)
#endif /* !QKING_NDEBUG */
                                 ...
                                 uint32_t options) /**< option bits */
{
  ...
  QKING_ASSERT (property_name_p != NULL);
  QKING_ASSERT (options == ECMA_PROPERTY_GET_NO_OPTIONS || property_ref_p != NULL);
    ...
    else if (ecma_is_normal_or_arrow_function (type))
    {
      if (ecma_string_is_length (property_name_p))
      {
        if (options & ECMA_PROPERTY_GET_VALUE)
        {
          ...
          size_t size = ((size_t)bytecode_data_p->size) << JMEM_ALIGNMENT_LOG;
            QKING_ASSERT(size == QKING_ALIGNUP(sizeof(ecma_compiled_function_state_t), JMEM_ALIGNMENT));
          uint32_t len = ((ecma_compiled_function_state_t *)bytecode_data_p)->argc;
                         ...
                         ecma_string_t *property_name_p) /**< property name */
{
...
#endif /* !CONFIG_DISABLE_ES2015_ARROW_FUNCTION */
        size_t size = ((size_t)bytecode_data_p->size) << JMEM_ALIGNMENT_LOG;
        QKING_ASSERT(size >= sizeof(ecma_compiled_function_state_t));
        uint32_t len = ((ecma_compiled_function_state_t *)bytecode_data_p)->argc;
  ...
  QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (*property_p) == ECMA_PROPERTY_TYPE_NAMEDACCESSOR);
  ...
  else
  {
        ...
        && ecma_get_object_extensible (object_p))
    {
      ...
      new_prop_value_p = ecma_create_named_data_property (object_p,
                                                          ...
                                                          NULL);
///////////////////////////////////////////////
      QKING_ASSERT (ecma_is_value_undefined (new_prop_value_p->value));
                       ...
                       bool is_throw) /**< flag that controls failure handling */
{
  ...
  QKING_ASSERT_OBJECT_TYPE_IS_VALID (ecma_get_object_type (obj_p));
                                    ...
                                    bool is_throw) /**< flag that controls failure handling */
{
  ...
  switch (type)
  {
    ...
    default:
    {
      QKING_ASSERT (type == ECMA_OBJECT_TYPE_PSEUDO_ARRAY);
                                            ...
                                            ecma_property_descriptor_t *prop_desc_p) /**< property descriptor */
{
  ...
  *prop_desc_p = ecma_make_empty_property_descriptor ();
  ...
  if (type != ECMA_PROPERTY_TYPE_NAMEDACCESSOR)
  {
    ...
    {
      QKING_ASSERT (type == ECMA_PROPERTY_TYPE_VIRTUAL);
                             ...
                             ecma_value_t value) /**< argument 'V' */
{
  ...
  QKING_ASSERT_OBJECT_TYPE_IS_VALID (type);
                                                                  ...
                                                                  *   false - list only own properties */
{
  ...
  const size_t bitmap_row_size = sizeof (uint32_t) * QKING_BITSINBYTE;
  const size_t names_hashes_bitmap_size = ECMA_OBJECT_HASH_BITMAP_SIZE / bitmap_row_size;
  QKING_VLA (uint32_t, names_hashes_bitmap, names_hashes_bitmap_size);
                                                        ...
                                                        : NULL)
  {
    ...
    else
    {
      switch (type)
      {
        ...
        default:
        {
          QKING_ASSERT (type == ECMA_OBJECT_TYPE_GENERAL);
    ...
    QKING_VLA (uint32_t, own_names_hashes_bitmap, own_names_hashes_bitmap_size);
    ...
    while (prop_iter_p != NULL)
    {
      ...
      for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
      {
            ...
            || ECMA_PROPERTY_GET_TYPE (*property_p) == ECMA_PROPERTY_TYPE_NAMEDACCESSOR)
        {
          ...
          {
            QKING_ASSERT (is_enumerable_only && !ecma_is_property_enumerable (*property_p));
    ...
    while (ecma_value_p != NULL)
    {
      ...
      if (index != ECMA_STRING_NOT_ARRAY_INDEX)
      {
        QKING_ASSERT (array_index_name_pos < array_index_named_properties_count);
        ...
        {
          QKING_ASSERT (insertion_pos < array_index_name_pos);
          QKING_ASSERT (index >= array_index_names_p[insertion_pos]);
         ...
         i++)
    {
      ...
      if (is_append)
      {
        QKING_ASSERT ((names_hashes_bitmap[bitmap_row] & (1u << bitmap_column)) != 0);
  ...
  switch (type)
  {
    ...
    case ECMA_OBJECT_TYPE_PSEUDO_ARRAY:
    {
      ...
      switch (ext_obj_p->u.pseudo_array.type)
      {
        ...
        default:
        {
          QKING_ASSERT (ext_obj_p->u.pseudo_array.type == ECMA_PSEUDO_ARRAY_ARGUMENTS);
    ...
    default:
    {
      ...
      if (ecma_get_object_is_builtin (obj_p))
      {
        ...
        switch (ext_obj_p->u.built_in.id)
        {
          ...
          default:
          {
            QKING_ASSERT (ecma_object_check_class_name_is_object (obj_p));
