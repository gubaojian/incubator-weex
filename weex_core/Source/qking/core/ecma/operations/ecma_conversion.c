...
ecma_op_to_string (ecma_value_t value) /**< ecma value */
{
  ...
  if (QKING_UNLIKELY (ecma_is_value_object (value)))
  {
...
ecma_op_from_property_descriptor (const ecma_property_descriptor_t *src_prop_desc_p) /**< property descriptor */
{
  ...
  {
    QKING_ASSERT (prop_desc.is_value_defined && prop_desc.is_writable_defined);
  ...
  {
    /* 4. */
    QKING_ASSERT (src_prop_desc_p->is_get_defined
    ...
    if (!ECMA_IS_VALUE_ERROR (ret_value))
    {
      ...
      if (ecma_is_value_found (get_prop_value))
      {
        ...
        else
        {
          ...
          {
            QKING_ASSERT (ecma_is_value_object (get_prop_value));
    ...
    if (!ECMA_IS_VALUE_ERROR (ret_value))
    {
      ...
      if (ecma_is_value_found (set_prop_value))
      {
        ...
        else
        {
          ...
          {
            QKING_ASSERT (ecma_is_value_object (set_prop_value));
...
/**
 * Raise a standard string with the given format string and arguments.
 *
 * @return ecma value
 *         Returned value must be freed with ecma_free_value
 */
ecma_value_t ecma_string_with_format (const char *format, /**< format string */
...) /**< ecma-values */
{
    QKING_ASSERT (format != NULL);
    ...
    while (*end_p)
    {
        if (*end_p == '%')
        {
            ...
            if (QKING_UNLIKELY (ecma_is_value_object (arg_val)))
            {
    ...
    return ecma_make_string_value(error_msg_p);
