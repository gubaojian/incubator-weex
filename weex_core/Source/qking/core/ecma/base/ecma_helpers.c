...
QKING_STATIC_ASSERT (ECMA_PROPERTY_TYPE_MASK >= ECMA_PROPERTY_TYPE__MAX,
                     ecma_property_types_must_be_lower_than_the_container_mask);

QKING_STATIC_ASSERT (ECMA_OBJECT_TYPE_MASK >= ECMA_OBJECT_TYPE__MAX - 1,
                     ecma_object_types_must_be_lower_than_the_container_mask);

QKING_STATIC_ASSERT (ECMA_OBJECT_TYPE_MASK >= ECMA_LEXICAL_ENVIRONMENT_TYPE__MAX,
                     ecma_lexical_environment_types_must_be_lower_than_the_container_mask);

QKING_STATIC_ASSERT (ECMA_OBJECT_TYPE_MASK + 1 == ECMA_OBJECT_FLAG_BUILT_IN_OR_LEXICAL_ENV,
                     ecma_built_in_flag_must_follow_the_object_type);

QKING_STATIC_ASSERT (ECMA_OBJECT_FLAG_EXTENSIBLE == (ECMA_OBJECT_FLAG_BUILT_IN_OR_LEXICAL_ENV << 1),
                     ecma_extensible_flag_must_follow_the_built_in_flag);

QKING_STATIC_ASSERT (ECMA_OBJECT_REF_ONE == (ECMA_OBJECT_FLAG_EXTENSIBLE << 1),
                     ecma_object_ref_one_must_follow_the_extensible_flag);

QKING_STATIC_ASSERT ((ECMA_OBJECT_MAX_REF | (ECMA_OBJECT_REF_ONE - 1)) == UINT16_MAX,
                     ecma_object_max_ref_does_not_fill_the_remaining_bits);

QKING_STATIC_ASSERT (ECMA_PROPERTY_TYPE_DELETED == (ECMA_DIRECT_STRING_MAGIC << ECMA_PROPERTY_NAME_TYPE_SHIFT),
                            ...
                            ecma_lexical_environment_type_t type) /**< type of the new lexical environment */
{
  ...
  QKING_ASSERT (type == ECMA_LEXICAL_ENVIRONMENT_THIS_OBJECT_BOUND
                || type == ECMA_LEXICAL_ENVIRONMENT_SUPER_OBJECT_BOUND);
#else /* CONFIG_DISABLE_ES2015_CLASS */
  QKING_ASSERT (type == ECMA_LEXICAL_ENVIRONMENT_THIS_OBJECT_BOUND);
#endif /* !CONFIG_DISABLE_ES2015_CLASS */

  QKING_ASSERT (binding_obj_p != NULL
...
{
  QKING_ASSERT (object_p != NULL);
  QKING_ASSERT (!ecma_is_lexical_environment (object_p));
 ...
 */
inline ecma_object_type_t QKING_ATTR_PURE
 ...
 */
inline ecma_object_t *QKING_ATTR_PURE
...
{
  QKING_ASSERT (object_p != NULL);
  QKING_ASSERT (!(object_p->type_flags_refs & ECMA_OBJECT_FLAG_BUILT_IN_OR_LEXICAL_ENV));
  QKING_ASSERT ((object_p->type_flags_refs & ECMA_OBJECT_TYPE_MASK) < ECMA_LEXICAL_ENVIRONMENT_TYPE_START);
 ...
 */
inline ecma_lexical_environment_type_t QKING_ATTR_PURE
 ...
 */
inline ecma_property_header_t *QKING_ATTR_PURE
...
ecma_get_lex_env_binding_object (const ecma_object_t *object_p) /**< object-bound lexical environment */
{
  ...
  QKING_ASSERT (ecma_get_lex_env_type (object_p) == ECMA_LEXICAL_ENVIRONMENT_THIS_OBJECT_BOUND
...
{
  QKING_ASSERT (ECMA_PROPERTY_PAIR_ITEM_COUNT == 2);
  ...
  if (*property_list_head_p != ECMA_NULL_POINTER)
  {
    ...
    QKING_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (first_property_p));
    ...
    if (first_property_p->types[0] == ECMA_PROPERTY_TYPE_DELETED)
    {
      ...
      QKING_ASSERT (ECMA_PROPERTY_VALUE_PTR (property_p) == first_property_pair_p->values + 0);
  ...
  QKING_ASSERT (ECMA_PROPERTY_VALUE_PTR (property_p) == first_property_pair_p->values + 1);
...
ecma_create_named_data_property (ecma_object_t *object_p, /**< object */
                                 ...
                                 ecma_property_t **out_prop_p) /**< [out] the property is also returned
                                                                *         if this field is non-NULL */
{
  QKING_ASSERT (object_p != NULL && name_p != NULL);
  QKING_ASSERT (ecma_find_named_property (object_p, name_p) == NULL);
  QKING_ASSERT ((prop_attributes & ~ECMA_PROPERTY_CONFIGURABLE_ENUMERABLE_WRITABLE) == 0);
                                                                    ...
                                                                    *         if this field is non-NULL */
{
  QKING_ASSERT (object_p != NULL && name_p != NULL);
  QKING_ASSERT (ecma_find_named_property (object_p, name_p) == NULL);
  QKING_ASSERT ((prop_attributes & ~ECMA_PROPERTY_CONFIGURABLE_ENUMERABLE) == 0);
                          ...
                          ecma_string_t *name_p) /**< property's name */
{
  ...
  if (ECMA_IS_DIRECT_STRING (name_p))
  {
    ...
    while (prop_iter_p != NULL)
    {
      ...
      {
        QKING_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (prop_iter_p->types[0]));
      ...
      {
        QKING_ASSERT (ECMA_PROPERTY_IS_NAMED_PROPERTY (prop_iter_p->types[1]));
...
{
  QKING_ASSERT (object_p != NULL && property_p != NULL);
  ...
  switch (ECMA_PROPERTY_GET_TYPE (*property_p))
  {
    ...
    default:
    {
      ...
      QKING_ASSERT (ECMA_PROPERTY_GET_NAME_TYPE (*property_p) == ECMA_DIRECT_STRING_MAGIC
                      ...
                      ecma_property_value_t *prop_value_p) /**< property value reference */
{
  ...
  while (true)
  {
    QKING_ASSERT (cur_prop_p != NULL);
    QKING_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (cur_prop_p));
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      ...
      {
        QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (cur_prop_p->types[i]) != ECMA_PROPERTY_TYPE_SPECIAL);
        ...
        QKING_ASSERT (cur_prop_p->types[i] == ECMA_PROPERTY_TYPE_DELETED);
...
{
  QKING_ASSERT (new_length < old_length);
  ...
  {
    QKING_ASSERT (ECMA_PROPERTY_IS_PROPERTY_PAIR (current_prop_p));
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
          ...
          && !ecma_is_property_configurable (current_prop_p->types[i]))
      {
        ...
        {
          QKING_ASSERT (index != ECMA_STRING_NOT_ARRAY_INDEX);
                                          ...
                                          ecma_property_types_t type) /**< expected property type */
{
  ...
  while (true)
  {
    ...
    for (int i = 0; i < ECMA_PROPERTY_PAIR_ITEM_COUNT; i++)
    {
      if ((prop_pair_p->values + i) == prop_value_p)
      {
        QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (prop_pair_p->header.types[i]) == type);
        return;
      }
    }
...
{
  QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (property) == ECMA_PROPERTY_TYPE_NAMEDDATA
 ...
 */
QKING_STATIC_ASSERT (sizeof (ecma_error_reference_t) == 8,
...
ecma_create_error_reference_from_context (void)
{
  ...
  ecma_value_t ret = ecma_create_error_reference (QKING_CONTEXT (error_value),
                                                  (QKING_CONTEXT (status_flags) & ECMA_STATUS_EXCEPTION) != 0);
  QKING_CONTEXT (error_value) = ECMA_VALUE_UNDEFINED;//ownership moved to  err ref;
  return ret;
 ...
 * @return error reference value
 */
inline ecma_value_t QKING_ATTR_ALWAYS_INLINE
...
{
  if (QKING_LIKELY (error_ref_p->refs_and_flags < ECMA_ERROR_MAX_REF))
  {
...
{
  QKING_ASSERT (error_ref_p->refs_and_flags >= ECMA_ERROR_REF_ONE);
...
ecma_bytecode_deref (ecma_compiled_code_t *bytecode_p) /**< byte code pointer */
{
  ...
  QKING_ASSERT (bytecode_p->refs > 0);
  QKING_ASSERT (!(bytecode_p->status_flags & ECMA_CODE_FLAGS_STATIC_FUNCTION));
  ...
  if (bytecode_p->status_flags & ECMA_CODE_FLAGS_FUNCTION)
  {
    ...
    if ((QKING_CONTEXT (debugger_flags) & QKING_DEBUGGER_CONNECTED)
        && !(bytecode_p->status_flags & ECMA_CODE_FLAGS_DEBUGGER_IGNORE)
        && qking_debugger_send_function_cp (QKING_DEBUGGER_RELEASE_BYTE_CODE_CP, bytecode_p))
    {
      /* Delay the byte code free until the debugger client is notified.
       * If the connection is aborted the pointer is still freed by
       * qking_debugger_close_connection(). */
      qking_debugger_byte_code_free_t *byte_code_free_p = (qking_debugger_byte_code_free_t *) bytecode_p;
      jmem_cpointer_t byte_code_free_head = QKING_CONTEXT (debugger_byte_code_free_head);
      ...
      {
        QKING_CONTEXT (debugger_byte_code_free_tail) = byte_code_free_cp;
      }
      else
      {
        qking_debugger_byte_code_free_t *first_byte_code_free_p;

        first_byte_code_free_p = JMEM_CP_GET_NON_NULL_POINTER (qking_debugger_byte_code_free_t,
                                                               byte_code_free_head);
        first_byte_code_free_p->prev_cp = byte_code_free_cp;
      }

      QKING_CONTEXT (debugger_byte_code_free_head) = byte_code_free_cp;
      return;
    }
#endif /* QKING_DEBUGGER */
