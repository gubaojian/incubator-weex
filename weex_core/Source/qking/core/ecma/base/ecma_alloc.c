...
QKING_STATIC_ASSERT (sizeof (ecma_property_value_t) == sizeof (ecma_value_t),
                     size_of_ecma_property_value_t_must_be_equal_to_size_of_ecma_value_t);
QKING_STATIC_ASSERT (((sizeof (ecma_property_value_t) - 1) & sizeof (ecma_property_value_t)) == 0,
                     size_of_ecma_property_value_t_must_be_power_of_2);

QKING_STATIC_ASSERT (sizeof (ecma_string_t) == sizeof (uint64_t),
                     size_of_ecma_string_t_must_be_less_than_or_equal_to_8_bytes);

//QKING_STATIC_ASSERT (sizeof (ecma_extended_object_t) - sizeof (ecma_object_t) <= sizeof (uint64_t), size_of_ecma_extended_object_part_must_be_less_than_or_equal_to_8_bytes);
 ...
 * @return pointer to allocated memory
 */
inline ecma_extended_object_t * QKING_ATTR_ALWAYS_INLINE
...
/**
 * Allocate memory for ecma-closure descriptor
 *
 * @return pointer to allocated memory
 */
inline ecma_closure_t * QKING_ATTR_ALWAYS_INLINE
ecma_alloc_closure (void)
{
    return (ecma_closure_t *) jmem_pools_alloc (sizeof (ecma_closure_t));
} /* ecma_alloc_closure */

/**
 * Dealloc memory from ecma-closure descriptor
 */
inline void QKING_ATTR_ALWAYS_INLINE
ecma_dealloc_closure (ecma_closure_t *closure_p) /**< closure to be freed */
{
    jmem_pools_free (closure_p, sizeof (ecma_closure_t));
} /* ecma_dealloc_closure */
 ...
 * @return pointer to allocated memory
 */
inline ecma_property_pair_t * QKING_ATTR_ALWAYS_INLINE
