...
QKING_STATIC_ASSERT (sizeof (ecma_value_t) == sizeof (ecma_integer_value_t),
                     size_of_ecma_value_t_must_be_equal_to_the_size_of_ecma_integer_value_t);

QKING_STATIC_ASSERT (ECMA_DIRECT_SHIFT == ECMA_VALUE_SHIFT + 1,
                     currently_directly_encoded_values_has_one_extra_flag);

QKING_STATIC_ASSERT (((1 << (ECMA_DIRECT_SHIFT - 1)) | ECMA_TYPE_DIRECT) == ECMA_DIRECT_TYPE_SIMPLE_VALUE,
...
QKING_STATIC_ASSERT (sizeof (ecma_number_t) == sizeof (uint32_t),
...
{
  QKING_ASSERT ((biased_exp & ~((1u << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1)) == 0);
  QKING_ASSERT ((fraction & ~((1ull << ECMA_NUMBER_FRACTION_WIDTH) - 1)) == 0);
...
QKING_STATIC_ASSERT (sizeof (ecma_number_t) == sizeof (uint64_t),
...
ecma_number_is_nan (ecma_number_t num) /**< ecma-number */
{
  ...
  QKING_ASSERT (is_nan == is_nan_ieee754);
#endif /* !QKING_NDEBUG */
...
{
  QKING_ASSERT (!ecma_number_is_nan (num));
...
ecma_number_is_zero (ecma_number_t num) /**< ecma-number */
{
  ...
  QKING_ASSERT (is_zero == is_zero_ieee754);
#endif /* !QKING_NDEBUG */
                                       ...
                                       int32_t *out_exponent_p) /**< [out] exponent of the number */
{
  ...
  if (QKING_UNLIKELY (biased_exp == 0))
  {
    /* IEEE-754 2008, 3.4, d */
  ...
  else if (ecma_number_is_infinity (num))
  {
    ...
    QKING_ASSERT ((fraction & (1ull << ECMA_NUMBER_FRACTION_WIDTH)) == 0);
  ...
  else
  {
    ...
    QKING_ASSERT (biased_exp > 0 && biased_exp < (1u << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1);
    QKING_ASSERT ((fraction & (1ull << ECMA_NUMBER_FRACTION_WIDTH)) == 0);
    fraction |= 1ull << ECMA_NUMBER_FRACTION_WIDTH;
  }
                                                             ...
                                                             int32_t exponent) /**< exponent */
{
  ...
  QKING_ASSERT (biased_exp > 0 && biased_exp < (1u << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1);
  QKING_ASSERT ((fraction & ~((1ull << (ECMA_NUMBER_FRACTION_WIDTH + 1)) - 1)) == 0);
  QKING_ASSERT ((fraction & (1ull << ECMA_NUMBER_FRACTION_WIDTH)) != 0);
                                                  ...
                                                  int32_t exponent) /**< binary exponent */
{
  ...
  if (mantissa & ~((1ull << (ECMA_NUMBER_FRACTION_WIDTH + 1)) - 1))
  {
    ...
    while ((mantissa & ~((1ull << (ECMA_NUMBER_FRACTION_WIDTH + 1)) - 1)) != 0)
    {
      ...
      if ((mantissa & ~((1ull << (ECMA_NUMBER_FRACTION_WIDTH + 1)) - 1)) == 0)
      {
        ...
        QKING_ASSERT (((mantissa & ~((1ull << (ECMA_NUMBER_FRACTION_WIDTH + 1)) - 1)) == 0)
  ...
  QKING_ASSERT (biased_exp < (1u << ECMA_NUMBER_BIASED_EXP_WIDTH) - 1);
  QKING_ASSERT ((mantissa & ~((1ull << ECMA_NUMBER_FRACTION_WIDTH) - 1)) == 0);
...
{
  QKING_ASSERT (!ecma_number_is_nan (num));
  QKING_ASSERT (!ecma_number_is_zero (num));
...
{
  QKING_ASSERT (!ecma_number_is_nan (num));
  QKING_ASSERT (!ecma_number_is_infinity (num));
  ...
  QKING_ASSERT (fraction & (1ull << ECMA_NUMBER_FRACTION_WIDTH));
...
{
  QKING_ASSERT (!ecma_number_is_nan (left_num)
                && !ecma_number_is_zero (left_num)
                && !ecma_number_is_infinity (left_num));
  QKING_ASSERT (!ecma_number_is_nan (right_num)
                       ...
                       ecma_integer_value_t right_integer) /**< right operand */
{
  ...
  if (QKING_UNLIKELY ((left_integer & (left_integer - 1)) == 0))
  {
  ...
  else if (QKING_UNLIKELY ((right_integer & (right_integer - 1)) == 0))
  {
