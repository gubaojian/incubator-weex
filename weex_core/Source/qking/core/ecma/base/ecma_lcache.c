...
{
  QKING_ASSERT (entry_p != NULL);
  QKING_ASSERT (entry_p->object_cp != ECMA_NULL_POINTER);
  QKING_ASSERT (entry_p->prop_p != NULL);
 ...
 * @return row index
 */
static inline size_t QKING_ATTR_ALWAYS_INLINE
                    ...
                    ecma_property_t *prop_p) /**< property */
{
  ...
  QKING_ASSERT (prop_p != NULL && !ecma_is_property_lcached (prop_p));
  QKING_ASSERT (ECMA_PROPERTY_GET_TYPE (*prop_p) == ECMA_PROPERTY_TYPE_NAMEDDATA
  ...
  ecma_lcache_hash_entry_t *entries_p = QKING_CONTEXT (lcache) [row_index];
 ...
 *         NULL otherwise
 */
inline ecma_property_t * QKING_ATTR_ALWAYS_INLINE
ecma_lcache_lookup (ecma_object_t *object_p, /**< object */
                    const ecma_string_t *prop_name_p) /**< property's name */
{
  QKING_ASSERT (object_p != NULL);
  QKING_ASSERT (prop_name_p != NULL);
  ...
  ecma_lcache_hash_entry_t *entry_p = QKING_CONTEXT (lcache) [row_index];
                        ...
                        ecma_property_t *prop_p) /**< property */
{
  ...
  while (true)
  {
    ...
    QKING_ASSERT (entry_p - QKING_CONTEXT (lcache) [row_index] < ECMA_LCACHE_HASH_ROW_LENGTH);

    if (entry_p->object_cp != ECMA_NULL_POINTER && entry_p->prop_p == prop_p)
    {
      QKING_ASSERT (entry_p->object_cp == object_cp);
