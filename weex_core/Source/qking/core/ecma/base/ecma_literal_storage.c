  ...
  while (string_list_p != NULL)
  {
    for (int i = 0; i < ECMA_LIT_STORAGE_VALUE_COUNT; i++)
    {
      if (string_list_p->values[i] != JMEM_CP_NULL)
      {
        ...
        QKING_ASSERT (ECMA_STRING_IS_REF_EQUALS_TO_ONE (string_p));
...
{
  ecma_free_string_list (QKING_CONTEXT (string_list_first_p));
  ecma_free_string_list (QKING_CONTEXT (number_list_first_p));
                                    ...
                                    lit_utf8_size_t size) /**< size of the string */
{
  ...
  ecma_lit_storage_item_t *string_list_p = QKING_CONTEXT (string_list_first_p);
  ...
  JMEM_CP_SET_POINTER (new_item_p->next_cp, QKING_CONTEXT (string_list_first_p));
  QKING_CONTEXT (string_list_first_p) = new_item_p;
...
ecma_find_or_create_literal_number (ecma_number_t number_arg) /**< number to be searched */
{
  ...
  QKING_ASSERT (ecma_is_value_float_number (num));

  ecma_lit_storage_item_t *number_list_p = QKING_CONTEXT (number_list_first_p);
  jmem_cpointer_t *empty_cpointer_p = NULL;
  ...
  while (number_list_p != NULL)
  {
    for (int i = 0; i < ECMA_LIT_STORAGE_VALUE_COUNT; i++)
    {
      ...
      else
      {
        ...
        QKING_ASSERT (ECMA_STRING_GET_CONTAINER (value_p) == ECMA_STRING_LITERAL_NUMBER);
        QKING_ASSERT (ecma_is_value_float_number (value_p->u.lit_number));
  ...
  JMEM_CP_SET_POINTER (new_item_p->next_cp, QKING_CONTEXT (number_list_first_p));
  QKING_CONTEXT (number_list_first_p) = new_item_p;
 ...
 */
#define QKING_SNAPSHOT_LITERAL_ALIGNMENT_LOG 1

/**
 * Snapshot literal alignment.
 */
#define QKING_SNAPSHOT_LITERAL_ALIGNMENT (1u << QKING_SNAPSHOT_LITERAL_ALIGNMENT_LOG)

/**
 * Literal offset shift.
 */
#define QKING_SNAPSHOT_LITERAL_SHIFT (ECMA_VALUE_SHIFT + 1)

/**
 * Literal value is number.
 */
#define QKING_SNAPSHOT_LITERAL_IS_NUMBER (1u << ECMA_VALUE_SHIFT)
                                 ...
                                 uint32_t *out_map_len_p) /**< [out] number of literals */
{
  ...
  if (max_lit_table_size > (UINT32_MAX >> QKING_SNAPSHOT_LITERAL_SHIFT))
  {
    max_lit_table_size = (UINT32_MAX >> QKING_SNAPSHOT_LITERAL_SHIFT);
  }
  ...
  while (iterator_p != NULL)
  {
    ...
    else
    {
      ...
      lit_table_size += (uint32_t) QKING_ALIGNUP (sizeof (uint16_t) + ecma_string_get_size (string_p),
                                                  QKING_SNAPSHOT_LITERAL_ALIGNMENT);
    }
  ...
  QKING_ASSERT ((*in_out_buffer_offset_p % sizeof (uint32_t)) == 0);
  ...
  *out_map_len_p = total_count;
  ...
  while (iterator_p != NULL)
  {
    ...
    map_p->literal_offset = (literal_offset << QKING_SNAPSHOT_LITERAL_SHIFT) | ECMA_TYPE_SNAPSHOT_OFFSET;

    ecma_length_t length;

    if (ecma_is_value_float_number (*iterator_p))
    {
      map_p->literal_offset |= QKING_SNAPSHOT_LITERAL_IS_NUMBER;

      ecma_number_t num = ecma_get_float_from_value (*iterator_p);
      memcpy (destination_p, &num, sizeof (ecma_number_t));

      length = QKING_ALIGNUP (sizeof (ecma_number_t), QKING_SNAPSHOT_LITERAL_ALIGNMENT);
    }
    else
    {
      ecma_string_t *string_p = ecma_get_string_from_value (*iterator_p);
      ...
      *(uint16_t *) destination_p = (uint16_t) length;
      ...
      length = QKING_ALIGNUP (sizeof (uint16_t) + length, QKING_SNAPSHOT_LITERAL_ALIGNMENT);
    }

    QKING_ASSERT ((length % sizeof (uint16_t)) == 0);
...
#if defined QKING_ENABLE_SNAPSHOT_EXEC || defined QKING_ENABLE_SNAPSHOT_SAVE
...
{
  QKING_ASSERT ((literal_value & ECMA_VALUE_TYPE_MASK) == ECMA_TYPE_SNAPSHOT_OFFSET);

  const uint8_t *literal_p = literal_base_p + (literal_value >> QKING_SNAPSHOT_LITERAL_SHIFT);

  if (literal_value & QKING_SNAPSHOT_LITERAL_IS_NUMBER)
  {
    ecma_number_t num;
...
#endif /* QKING_ENABLE_SNAPSHOT_EXEC || QKING_ENABLE_SNAPSHOT_SAVE */
