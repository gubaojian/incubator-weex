...
QKING_STATIC_ASSERT (ECMA_STRING_CONTAINER_MASK + 1 == ECMA_STRING_REF_ONE,
                     ecma_string_ref_counter_should_start_after_the_container_field);

QKING_STATIC_ASSERT (ECMA_STRING_CONTAINER_MASK >= ECMA_STRING_CONTAINER__MAX,
                     ecma_string_container_types_must_be_lower_than_the_container_mask);

QKING_STATIC_ASSERT ((ECMA_STRING_MAX_REF | ECMA_STRING_CONTAINER_MASK) == UINT16_MAX,
                     ecma_string_ref_and_container_fields_should_fill_the_16_bit_field);

QKING_STATIC_ASSERT (ECMA_STRING_NOT_ARRAY_INDEX == UINT32_MAX,
                     ecma_string_not_array_index_must_be_equal_to_uint32_max);

QKING_STATIC_ASSERT ((ECMA_TYPE_DIRECT_STRING & 0x1) != 0,
                     ecma_type_direct_string_must_be_odd_number);

QKING_STATIC_ASSERT (LIT_MAGIC_STRING__COUNT <= ECMA_DIRECT_STRING_MAX_IMM,
                     all_magic_strings_must_be_encoded_as_direct_string);

QKING_STATIC_ASSERT ((int) ECMA_DIRECT_STRING_UINT == (int) ECMA_STRING_CONTAINER_UINT32_IN_DESC
                     && (int) ECMA_DIRECT_STRING_MAGIC_EX == (int) ECMA_STRING_CONTAINER_MAGIC_STRING_EX,
                     ecma_direct_and_container_types_must_match);

QKING_STATIC_ASSERT (ECMA_PROPERTY_NAME_TYPE_SHIFT > ECMA_VALUE_SHIFT,
...
{
  QKING_ASSERT (string_size > 0 && *string_p >= LIT_CHAR_0 && *string_p <= LIT_CHAR_9);
                            ...
                            lit_utf8_size_t *size_p) /**< [out] size of the ecma string */
{
  if (ECMA_IS_DIRECT_STRING (string_p))
  {
    ...
    QKING_ASSERT (ECMA_GET_DIRECT_STRING_TYPE (string_p) == ECMA_DIRECT_STRING_MAGIC_EX);
  ...
  QKING_ASSERT (string_p->refs_and_container >= ECMA_STRING_REF_ONE);
  ...
  switch (ECMA_STRING_GET_CONTAINER (string_p))
  {
    ...
    default:
    {
      QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string_p) == ECMA_STRING_CONTAINER_MAGIC_STRING_EX);
...
{
  QKING_ASSERT (id < lit_get_magic_string_ex_count ());

  if (QKING_LIKELY (id <= ECMA_DIRECT_STRING_MAX_IMM))
  {
...
ecma_new_ecma_string_from_utf8 (const lit_utf8_byte_t *string_p, /**< utf-8 string */
                                lit_utf8_size_t string_size) /**< string size */
{
  QKING_ASSERT (string_p != NULL || string_size == 0);
  QKING_ASSERT (lit_is_valid_cesu8_string (string_p, string_size));
  ...
  if (QKING_LIKELY (string_size <= UINT16_MAX))
  {
                                                   ...
                                                   lit_utf8_size_t string_size) /**< utf-8 string size */
{
  ...
  while (pos < string_size)
  {
    ...
    {
      QKING_ASSERT ((string_p[pos] & LIT_UTF8_4_BYTE_MASK) == LIT_UTF8_4_BYTE_MARKER);
      pos += 4;
  ...
  else
  {
    ...
    QKING_ASSERT (lit_is_valid_utf8_string (string_p, string_size));

    lit_utf8_byte_t *data_p;

    if (QKING_LIKELY (converted_string_size <= UINT16_MAX))
    {
    ...
    while (pos < string_size)
    {
      if ((string_p[pos] & LIT_UTF8_4_BYTE_MASK) == LIT_UTF8_4_BYTE_MARKER)
      {
        ...
        QKING_ASSERT (character >= 0x10000);
        character -= 0x10000;
...
{
  if (QKING_LIKELY (uint32_number <= ECMA_DIRECT_STRING_MAX_IMM))
  {
...
{
  QKING_ASSERT (uint32_number <= ECMA_DIRECT_STRING_MAX_IMM);
...
ecma_new_ecma_string_from_number (ecma_number_t num) /**< ecma-number */
{
  ...
  QKING_ASSERT (str_size > 0);
#ifndef QKING_NDEBUG
  QKING_ASSERT (lit_is_utf8_string_magic (str_buf, str_size) == LIT_MAGIC_STRING__COUNT
 ...
 * @return pointer to ecma-string descriptor
 */
inline ecma_string_t * QKING_ATTR_ALWAYS_INLINE
ecma_get_magic_string (lit_magic_string_id_t id) /**< identifier of magic string */
{
  QKING_ASSERT (id < LIT_MAGIC_STRING__COUNT);
...
{
  QKING_ASSERT (string1_p != NULL && cesu8_string2_size > 0 && cesu8_string2_length > 0);

  if (QKING_UNLIKELY (ecma_string_is_empty (string1_p)))
  {
  ...
  if (ECMA_IS_DIRECT_STRING (string1_p))
  {
    ...
    switch (ECMA_GET_DIRECT_STRING_TYPE (string1_p))
    {
      ...
      default:
      {
        QKING_ASSERT (ECMA_GET_DIRECT_STRING_TYPE (string1_p) == ECMA_DIRECT_STRING_MAGIC_EX);
  ...
  {
    QKING_ASSERT (string1_p->refs_and_container >= ECMA_STRING_REF_ONE);
    ...
    switch (ECMA_STRING_GET_CONTAINER (string1_p))
    {
      ...
      default:
      {
        QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string1_p) == ECMA_STRING_CONTAINER_MAGIC_STRING_EX);
  ...
  QKING_ASSERT (cesu8_string1_length > 0);
  QKING_ASSERT (cesu8_string1_length <= cesu8_string1_size);
  ...
  if (QKING_LIKELY (new_size <= UINT16_MAX))
  {
  ...
  {
    QKING_ASSERT (!ECMA_IS_DIRECT_STRING (string1_p));
...
{
  QKING_ASSERT (string1_p != NULL && string2_p != NULL);
  ...
  else if (QKING_UNLIKELY (ecma_string_is_empty (string2_p)))
  {
    return string1_p;
  }
  ...
  if (ECMA_IS_DIRECT_STRING (string2_p))
  {
    switch (ECMA_GET_DIRECT_STRING_TYPE (string2_p))
    {
      ...
      default:
      {
        QKING_ASSERT (ECMA_GET_DIRECT_STRING_TYPE (string2_p) == ECMA_DIRECT_STRING_MAGIC_EX);
  ...
  {
    QKING_ASSERT (string2_p->refs_and_container >= ECMA_STRING_REF_ONE);
    ...
    switch (ECMA_STRING_GET_CONTAINER (string2_p))
    {
      ...
      default:
      {
        QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string2_p) == ECMA_STRING_CONTAINER_MAGIC_STRING_EX);
...
ecma_ref_ecma_string (ecma_string_t *string_p) /**< string descriptor */
{
  ...
  if (ECMA_IS_DIRECT_STRING (string_p))
  {
    ...
    uint16_t ref = (uint16_t) (string_p->refs_and_container) >> 3;
    printf("[gc][%i]=>[copy][%i]\n", ecma_make_string_value(string_p), ref + 1);
#endif

  QKING_ASSERT (string_p->refs_and_container >= ECMA_STRING_REF_ONE);

  if (QKING_LIKELY (string_p->refs_and_container < ECMA_STRING_MAX_REF))
  {
  ...
  switch (ECMA_STRING_GET_CONTAINER (string_p))
  {
    case ECMA_STRING_CONTAINER_HEAP_UTF8_STRING:
    {
      ...
      if (*chars_p >= LIT_CHAR_0 && *chars_p <= LIT_CHAR_9)
      {
        ...
        QKING_ASSERT (!ecma_string_to_array_index (chars_p,
    ...
    case ECMA_STRING_CONTAINER_HEAP_LONG_UTF8_STRING:
    {
      QKING_ASSERT (string_p->u.long_utf8_string_size > UINT16_MAX);
    ...
    default:
    {
      QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string_p) == ECMA_STRING_CONTAINER_UINT32_IN_DESC
 ...
 */
inline uint32_t QKING_ATTR_ALWAYS_INLINE
 ...
 */
lit_utf8_size_t QKING_ATTR_WARN_UNUSED_RESULT
...
{
  QKING_ASSERT (string_p != NULL);
  QKING_ASSERT (buffer_p != NULL || buffer_size == 0);
  QKING_ASSERT (ecma_string_get_size (string_p) <= buffer_size);
                                 ...
                                 lit_utf8_size_t buffer_size) /**< size of buffer */
{
  QKING_ASSERT (string_p != NULL);
  QKING_ASSERT (buffer_p != NULL || buffer_size == 0);
  QKING_ASSERT (ecma_string_get_utf8_size (string_p) <= buffer_size);
...
ecma_substring_copy_to_cesu8_buffer (const ecma_string_t *string_desc_p, /**< ecma-string descriptor */
                                     ...
                                     lit_utf8_size_t buffer_size) /**< size of buffer */
{
  QKING_ASSERT (string_desc_p != NULL);
  QKING_ASSERT (buffer_p != NULL || buffer_size == 0);
...
{
  QKING_ASSERT (string_desc_p != NULL);
  QKING_ASSERT (ECMA_IS_DIRECT_STRING (string_desc_p) || string_desc_p->refs_and_container >= ECMA_STRING_REF_ONE);
  QKING_ASSERT (buffer_p != NULL || buffer_size == 0);
  ...
  else
  {
    ...
    while (start_pos--)
    {
      ...
      if ((cesu8_str_p != cesu8_end_pos) && lit_is_code_point_utf16_high_surrogate (ch))
      {
        ...
        {
          QKING_ASSERT (code_unit_size == next_ch_size);
          cesu8_str_p += code_unit_size;
        }
      }
    }
    ...
    QKING_ASSERT (utf8_pos <= utf8_end_pos);
  }
 ...
 */
static inline ecma_length_t QKING_ATTR_ALWAYS_INLINE
  ...
  if (ECMA_IS_DIRECT_STRING (string_p))
  {
    switch (ECMA_GET_DIRECT_STRING_TYPE (string_p))
    {
      case ECMA_DIRECT_STRING_MAGIC:
      {
        ...
        QKING_ASSERT (ECMA_STRING_IS_ASCII (result_p, size));
        break;
      }
      case ECMA_DIRECT_STRING_UINT:
      {
      ...
      default:
      {
        ...
        if (QKING_UNLIKELY (*flags_p & ECMA_STRING_FLAG_IS_ASCII))
        {
 ...
 */
static inline ecma_string_t * QKING_ATTR_ALWAYS_INLINE
 ...
 */
inline jmem_cpointer_t QKING_ATTR_ALWAYS_INLINE
 ...
 */
inline lit_string_hash_t QKING_ATTR_ALWAYS_INLINE
 ...
 *         false - otherwise
 */
static bool QKING_ATTR_NOINLINE
ecma_compare_ecma_strings_longpath (const ecma_string_t *string1_p, /**< ecma-string */
                                    const ecma_string_t *string2_p) /**< ecma-string */
{
  QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string1_p) == ECMA_STRING_GET_CONTAINER (string2_p));
  ...
  {
    QKING_ASSERT (ECMA_STRING_GET_CONTAINER (string1_p) == ECMA_STRING_CONTAINER_HEAP_LONG_UTF8_STRING);
...
{
  QKING_ASSERT (string1_p != NULL && string2_p != NULL);
  QKING_ASSERT (!ECMA_IS_DIRECT_STRING (string1_p) && !ECMA_IS_DIRECT_STRING (string2_p));
  ...
  if (ECMA_IS_DIRECT_STRING (string_p))
  {
    switch (ECMA_GET_DIRECT_STRING_TYPE (string_p))
    {
      case ECMA_DIRECT_STRING_MAGIC:
      {
        ...
        QKING_ASSERT (ECMA_STRING_IS_ASCII (lit_get_magic_string_utf8 (id),
...
{
  QKING_ASSERT (index < ecma_string_get_length (string_p));
                    ...
                    ecma_length_t end_pos) /**< end position, should be less or equal than string length */
{
  ...
  QKING_ASSERT (start_pos <= string_length);
  QKING_ASSERT (end_pos <= string_length);
