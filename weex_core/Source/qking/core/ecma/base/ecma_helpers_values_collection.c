 ...
 */
QKING_STATIC_ASSERT (ECMA_TYPE_ERROR == ECMA_TYPE_POINTER,
                                  ...
                                  uint32_t flags) /**< combination of ecma_collection_flag_t flags */
{
  ...
  if (QKING_UNLIKELY (header_p->item_count == 0))
  {
    item_index = 0;
  ...
  else
  {
    ...
    if (QKING_UNLIKELY (item_index == 0))
    {
      QKING_ASSERT (ecma_is_value_pointer (chunk_p->items[ECMA_COLLECTION_CHUNK_ITEMS])
    ...
    {
      QKING_ASSERT (ecma_is_value_pointer (chunk_p->items[item_index])
...
{
  if (QKING_UNLIKELY (!header_p || header_p->item_count == 0))
  {
    return NULL;
  }
...
{
  QKING_ASSERT (ecma_value_p != NULL);

  ecma_value_p++;

  if (QKING_UNLIKELY (ecma_is_value_pointer (*ecma_value_p)))
  {
    ecma_value_p = ((ecma_collection_chunk_t *) ecma_get_pointer_from_value (*ecma_value_p))->items;

    QKING_ASSERT (ecma_value_p == NULL || !ecma_is_value_pointer (*ecma_value_p));
    return ecma_value_p;
  }
