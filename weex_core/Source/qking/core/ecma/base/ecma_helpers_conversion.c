 ...
 */
static inline int QKING_ATTR_ALWAYS_INLINE
                            ...
                            lit_utf8_size_t str_size) /**< string size */
{
  ...
  QKING_ASSERT (strlen ((const char *) infinity_zt_str_p) == 8);
  ...
  if (!e_sign)
  {
    ...
    while (e > 0)
    {
      QKING_ASSERT (ECMA_UINT128_CLZ_MAX63 (fraction_uint128) == 4);
  ...
  else
  {
    ...
    while (e > 0)
    {
      ...
      QKING_ASSERT (shift <= 4);
      ECMA_UINT128_LEFT_SHIFT_MAX63 (fraction_uint128, shift);
      binary_exponent -= shift;

      QKING_ASSERT (!ECMA_UINT128_IS_ZERO (fraction_uint128));
  ...
  QKING_ASSERT (ECMA_UINT128_CLZ_MAX63 (fraction_uint128) == 11);
                            ...
                            lit_utf8_size_t buffer_size) /**< size of buffer */
{
  ...
  do
  {
    QKING_ASSERT (buf_p >= out_buffer_p);
  ...
  if (QKING_LIKELY (buf_p != out_buffer_p))
  {
...
ecma_number_to_uint32 (ecma_number_t num) /**< ecma-number */
{
  ...
  QKING_ASSERT (num_in_uint32_range < uint64_2_pow_32);
  ...
  if (sign
      && uint32_num != 0)
  {
    QKING_ASSERT (ret == uint64_2_pow_32 - uint32_num);
  }
  else
  {
    QKING_ASSERT (ret == uint32_num);
  }
#endif /* !QKING_NDEBUG */
...
ecma_number_to_int32 (ecma_number_t num) /**< ecma-number */
{
  ...
  QKING_ASSERT (int64_num >= 0);

  if (int64_num >= uint32_2_pow_31)
  {
    QKING_ASSERT (ret == int64_num - int64_2_pow_32);
  }
  else
  {
    QKING_ASSERT (ret == int64_num);
  }
#endif /* !QKING_NDEBUG */
                        ...
                        int32_t *out_decimal_exp_p) /**< [out] decimal exponent */
{
  ...
  QKING_ASSERT (!ecma_number_is_negative (num));
 ...
 */
inline static int32_t QKING_ATTR_ALWAYS_INLINE
ecma_number_of_digits (double val) /**< ecma number */
{
  QKING_ASSERT (fabs (fmod (val, 1.0)) < EPSILON);
  int32_t exponent = 0;
 ...
 */
inline static void QKING_ATTR_ALWAYS_INLINE
 ...
 */
static inline lit_utf8_size_t QKING_ATTR_ALWAYS_INLINE
                            ...
                            lit_utf8_size_t buffer_size) /**< size of buffer */
{
  ...
  if (ecma_number_is_infinity (num))
  {
    ...
    QKING_ASSERT (dst_p <= buffer_p + buffer_size);
    return (lit_utf8_size_t) (dst_p - buffer_p);
  }

  QKING_ASSERT (ecma_number_get_next (ecma_number_get_prev (num)) == num);
