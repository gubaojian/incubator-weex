...
#include "lit_unicode_ranges.inc.h"
#include "lit_strings.h"

#ifndef CONFIG_DISABLE_UNICODE_CASE_CONVERSION
#include "lit_unicode_conversions.inc.h"
...
{
  QKING_ASSERT (lit_char_is_hex_digit (c));
                             ...
                             ecma_char_t *out_code_unit_p) /**< [out] decoded result */
{
  ...
  QKING_ASSERT (number_of_characters >= 2 && number_of_characters <= 4);
...
{
  QKING_ASSERT (buffer_size >= LIT_MAXIMUM_OTHER_CASE_LENGTH);
