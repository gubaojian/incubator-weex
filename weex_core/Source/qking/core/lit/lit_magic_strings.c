...
{
  return QKING_CONTEXT (lit_magic_string_ex_count);
...
{
  static const lit_utf8_byte_t * const lit_magic_strings[] QKING_CONST_DATA =
  {
...
#include "lit_magic_strings.inc.h"
#undef LIT_MAGIC_STRING_DEF
#undef LIT_MAGIC_STRING_FIRST_STRING_WITH_SIZE
/** @endcond */
  };

  QKING_ASSERT (id < LIT_NON_INTERNAL_MAGIC_STRING__COUNT);
...
{
  static const lit_magic_size_t lit_magic_string_sizes[] QKING_CONST_DATA =
  {
/** @cond doxygen_suppress */
#define LIT_MAGIC_STRING_FIRST_STRING_WITH_SIZE(size, id)
#define LIT_MAGIC_STRING_DEF(id, utf8_string) \
...
{
  static const lit_magic_string_id_t lit_magic_string_size_block_starts[] QKING_CONST_DATA =
  {
/** @cond doxygen_suppress */
#define LIT_MAGIC_STRING_DEF(id, utf8_string)
#define LIT_MAGIC_STRING_FIRST_STRING_WITH_SIZE(size, id) \
...
/** @endcond */
  ...
  QKING_ASSERT (size <= (sizeof (lit_magic_string_size_block_starts) / sizeof (lit_magic_string_id_t)));
...
{
  QKING_ASSERT (QKING_CONTEXT (lit_magic_string_ex_array) && id < QKING_CONTEXT (lit_magic_string_ex_count));

  return QKING_CONTEXT (lit_magic_string_ex_array)[id];
...
{
  return QKING_CONTEXT (lit_magic_string_ex_sizes)[id];
...
{
  QKING_ASSERT (ex_str_items != NULL);
  QKING_ASSERT (count > 0);
  QKING_ASSERT (ex_str_sizes != NULL);

  QKING_ASSERT (QKING_CONTEXT (lit_magic_string_ex_array) == NULL);
  QKING_ASSERT (QKING_CONTEXT (lit_magic_string_ex_count) == 0);
  QKING_ASSERT (QKING_CONTEXT (lit_magic_string_ex_sizes) == NULL);

  /* Set external magic strings information */
  QKING_CONTEXT (lit_magic_string_ex_array) = ex_str_items;
  QKING_CONTEXT (lit_magic_string_ex_count) = count;
  QKING_CONTEXT (lit_magic_string_ex_sizes) = ex_str_sizes;

#ifndef QKING_NDEBUG
  for (lit_magic_string_ex_id_t id = (lit_magic_string_ex_id_t) 0;
       id < QKING_CONTEXT (lit_magic_string_ex_count);
       id = (lit_magic_string_ex_id_t) (id + 1))
  {
    lit_utf8_size_t string_size = QKING_CONTEXT (lit_magic_string_ex_sizes)[id];
    ...
    if (id > 0)
    {
      ...
      QKING_ASSERT (lit_is_valid_cesu8_string (lit_get_magic_string_ex_utf8 (id),
                                               string_size));
      QKING_ASSERT (prev_string_size <= string_size);
      ...
      if (prev_string_size == string_size)
      {
        ...
        QKING_ASSERT (memcmp (prev_ex_string_p, curr_ex_string_p, string_size) < 0);
      }
    }
  }
#endif /* !QKING_NDEBUG */
                                 ...
                                 lit_utf8_size_t buffer_size) /**< size of buffer */
{
  ...
  {
    bytes_copied ++;
    QKING_ASSERT (bytes_copied <= buffer_size);
