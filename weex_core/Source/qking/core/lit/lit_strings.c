...
{
  QKING_ASSERT (code_point > LIT_UTF16_CODE_UNIT_MAX);
...
{
  QKING_ASSERT (code_point > LIT_UTF16_CODE_UNIT_MAX);
  QKING_ASSERT (code_point <= LIT_UNICODE_CODE_POINT_MAX);
                        ...
                        lit_utf8_size_t utf8_buf_size) /**< string size */
{
  ...
  QKING_ASSERT (size == utf8_buf_size);
                                   ...
                                   lit_utf8_size_t cesu8_buf_size) /**< string size */
{
  ...
  QKING_ASSERT (offset == cesu8_buf_size);
                               ...
                               lit_code_point_t *code_point) /**< [out] code point */
{
  ...
  {
    QKING_ASSERT ((c & LIT_UTF8_4_BYTE_MASK) == LIT_UTF8_4_BYTE_MARKER);
    bytes_count = 4;
    ret = ((lit_code_point_t) (c & LIT_UTF8_LAST_3_BITS_MASK));
  }

  QKING_ASSERT (buf_size >= bytes_count);
                              ...
                              ecma_char_t *code_point) /**< [out] code point */
{
  ...
  {
    QKING_ASSERT ((c & LIT_UTF8_3_BYTE_MASK) == LIT_UTF8_3_BYTE_MARKER);
    bytes_count = 3;
    ret = ((lit_code_point_t) (c & LIT_UTF8_LAST_4_BITS_MASK));
  }
  ...
  QKING_ASSERT (ret <= LIT_UTF16_CODE_UNIT_MAX);
...
{
  QKING_ASSERT (utf8_buf_p != NULL || utf8_buf_size == 0);
                              ...
                              ecma_length_t code_unit_offset) /**< ofset of a code_unit */
{
  ...
  do
  {
    QKING_ASSERT (current_p < utf8_buf_p + utf8_buf_size);
 ...
 */
inline lit_utf8_size_t QKING_ATTR_ALWAYS_INLINE
lit_get_unicode_char_size_by_utf8_first_byte (const lit_utf8_byte_t first_byte) /**< buffer with characters */
{
  ...
  {
    QKING_ASSERT ((first_byte & LIT_UTF8_3_BYTE_MASK) == LIT_UTF8_3_BYTE_MARKER);
    return 3;
  }
                                                ...
                                                *   should be at least LIT_UTF8_MAX_BYTES_IN_CODE_UNIT */
{
  ...
  else if (code_unit <= LIT_UTF8_2_BYTE_CODE_POINT_MAX)
  {
    ...
    QKING_ASSERT (first_byte_bits == code_unit_bits);
                                              ...
                                              *   its size should be at least 4 bytes */
{
  ...
  else if (code_point <= LIT_UTF8_2_BYTE_CODE_POINT_MAX)
  {
    ...
    QKING_ASSERT (first_byte_bits == code_point_bits);
  ...
  {
    QKING_ASSERT (code_point <= LIT_UTF8_4_BYTE_CODE_POINT_MAX);
                                         ...
                                         lit_utf8_size_t utf8_size) /**< size of utf-8 buffer */
{
  ...
  while (cesu8_pos < cesu8_end_pos)
  {
    ...
    if (lit_is_code_point_utf16_low_surrogate (ch) && lit_is_code_point_utf16_high_surrogate (prev_ch))
    {
      QKING_ASSERT (code_unit_size == prev_ch_size);
  ...
  QKING_ASSERT (cesu8_pos == cesu8_end_pos);
...
{
  QKING_ASSERT (lit_is_code_point_utf16_high_surrogate (high_surrogate));
  QKING_ASSERT (lit_is_code_point_utf16_low_surrogate (low_surrogate));
