...
#ifndef QKING_NDEBUG
    for (int i = 0; i < decoder->string_array_size; i++) {
       qking_release_value(decoder->string_array[i]);
    }
#endif
...
#ifndef QKING_NDEBUG
    for (int i = 0; i < decoder->regex_flag_array_size; i++) {
        qking_release_value(decoder->regex_flag_array[i]);
    }
#endif
